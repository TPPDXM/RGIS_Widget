// FreqDomainTwoDerivDlg.h : 频率域二阶导数对话框（Qt 前端）
//
// 对应原 MFC 工程：CFreqDomainTwoDerivDlg（IDD_FreqDomainTwoDeriv）
// 界面布局依据 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainTwoDeriv 绘制：
//   数据文件输入 / 网格数据信息 / 数据扩边信息 / 数据扩边方法 / 处理参数 / 数据文件输出
//
// 前端职责（本对话框只做界面与交互，算法/数据读写在后端）：
//   1. 输入数据文件的选择、网格信息显示、默认输出文件名生成（_HDR_II.grd）；
//   2. 扩边行数/列数（2 的幂）微调、扩边方法单选（4 种）、导数倾角/导数偏角输入；
//   3. “确定”时校验输入，并调用后端接口 processTwoDeriv 完成处理；
...
// 频率域二阶导数对话框
class CFreqDomainTwoDerivDlg : public QDialog
{
    Q_OBJECT
...
public:
    ...
    explicit CFreqDomainTwoDerivDlg(QWidget* pParent = nullptr);

    // 析构函数
    ~CFreqDomainTwoDerivDlg() override;
...
private slots:
    ...
    // ===== 处理参数回调 =====
    void onDerivIncValueChanged(double dValue); // 功能：导数倾角变化（同步成员变量）
    void onDerivDecValueChanged(double dValue); // 功能：导数偏角变化（同步成员变量）
...
private:
    ...
    void runProcess();                          // 功能：组装参数并调用后端 processTwoDeriv
    ...
    // ===== 界面控件（处理参数，对应 .rc 中 IDC_DerivInc / IDC_DerivDec）=====
    QDoubleSpinBox* mSpinDerivInc;   // 导数倾角（度，范围 0~360，默认 0.0）
    QDoubleSpinBox* mSpinDerivDec;   // 导数偏角（度，范围 0~360，默认 0.0）

    // ===== 界面控件（数据文件输出）=====
    QLineEdit*  mEditSaveFile;       // 处理结果数据文件名（默认 基准名 + "_HDR_II.grd"）
    ...
    double  mDerivInc;      // 导数倾角（原 m_fDerivInc，默认 0.0）
    double  mDerivDec;      // 导数偏角（原 m_fDerivDec，默认 0.0）
};
