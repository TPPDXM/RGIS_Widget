// FreqDomainDzToPoleDlg.h : 频率域 ΔZ 化极（Za 化极）对话框（Qt 前端）
//
// 对应原 MFC 工程：CFreqDomainDzToPoleDlg（IDD_FreqDomainDzToPole）
// 界面布局依据 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainDzToPole 绘制：
...
//   2. 扩边行数/列数微调、磁化偏角/磁化倾角输入；
//   3. “确定”时校验输入，并调用后端接口 processDzToPole 完成处理；
...
// 频率域 ΔZ 化极（Za 化极）对话框
class CFreqDomainDzToPoleDlg : public QDialog
{
    Q_OBJECT
...
public:
    ...
    explicit CFreqDomainDzToPoleDlg(QWidget* pParent = nullptr);

    // 析构函数
    ~CFreqDomainDzToPoleDlg() override;
...
private:
    ...
    void runProcess();                          // 功能：组装参数并调用后端 processDzToPole
    ...
    // ===== 界面控件（处理参数，对应 .rc 中 IDC_DTD0 / IDC_DTI0）=====
    QDoubleSpinBox* mSpinDtd0;      // 磁化偏角（度，范围 -360~360，默认 30.0）
    QDoubleSpinBox* mSpinDti0;      // 磁化倾角（度，范围 -90~90，默认 60.0）
    ...
    double  mDtd0;          // 磁化偏角（原 m_fDtD0，默认 30.0）
    double  mDti0;          // 磁化倾角（原 m_fDtI0，默认 60.0）
};
