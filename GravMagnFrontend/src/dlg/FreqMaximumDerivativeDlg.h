// FreqMaximumDerivativeDlg.h : 频率域最大导数（最大水平方向导数）对话框（Qt 前端）
//
// 对应原 MFC 工程：CFreqMaximumDerivativeDlg（IDD_FreqMaximumDerivative）
// 界面布局依据 RGISGravMagnDataProcessing.rc 中 IDD_FreqMaximumDerivative 绘制：
//   网格数据文件输入 / 网格数据信息 / 处理参数 / 网格数据文件输出
// 说明：原工程此对话框无扩边信息（扩边由算法内部完成）。
//
// 前端职责（本对话框只做界面与交互，算法/数据读写在后端）：
//   1. 输入数据文件的选择、网格信息显示、默认输出文件名生成（_MaxHDR.grd）；
//   2. 水平方向角度间隔输入；
//   3. “确定”时校验输入，并调用后端接口 processMaximumDerivative 完成处理；
...
// 频率域最大导数（最大水平方向导数）对话框
class CFreqMaximumDerivativeDlg : public QDialog
{
    Q_OBJECT
...
public:
    ...
    explicit CFreqMaximumDerivativeDlg(QWidget* pParent = nullptr);

    // 析构函数
    ~CFreqMaximumDerivativeDlg() override;
...
private slots:
    ...
    // ===== 处理参数回调 =====
    void onAzimuthStepValueChanged(int nValue); // 功能：水平方向角度间隔变化（同步成员变量）
...
private:
    ...
    void runProcess();                          // 功能：组装参数并调用后端 processMaximumDerivative
    ...
    // ===== 界面控件（处理参数，对应 .rc 中 IDC_AzimuthStep）=====
    QSpinBox*   mSpinAzimuthStep;   // 水平方向角度间隔（度/步长，范围 1~20，默认 5）

    // ===== 界面控件（网格数据文件输出）=====
    QLineEdit*  mEditSaveFile;       // 处理结果数据文件名（默认 基准名 + "_MaxHDR.grd"）
    ...
    int     mAzimuthStep;   // 水平方向角度间隔（原 m_nAzimuthStep，默认 5）
};
