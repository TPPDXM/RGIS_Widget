// FreqDomainOneDerivDlg.cpp : 频率域一阶导数对话框实现
//
// 说明：
//   * 界面布局对照 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainOneDeriv；
//   * 所有回调均为成员函数引用（connect 的第四个参数为 &类名::槽函数），不使用 lambda；
//   * 全程不使用 try/catch，错误一律通过返回值 + 消息框反馈，做足空指针检查。

#include "FreqDomainOneDerivDlg.h"
...
CFreqDomainOneDerivDlg::CFreqDomainOneDerivDlg(QWidget* pParent)
    : QDialog(pParent)
    , mEditOpenFile(NULL)
    , mBtnOpenFile(NULL)
...
CFreqDomainOneDerivDlg::~CFreqDomainOneDerivDlg()
{
}

// 功能：按 .rc 布局创建全部控件并连接信号槽
void CFreqDomainOneDerivDlg::initUi()
{
    setWindowTitle(QStringLiteral("频率域一阶导数"));
    setModal(true);
    ...
    // 输入文件
    connect(mBtnOpenFile, &QPushButton::clicked, this, &CFreqDomainOneDerivDlg::onOpenFileClicked);
    connect(mBtnOpenFileView, &QPushButton::clicked, this, &CFreqDomainOneDerivDlg::onOpenFileViewClicked);
    connect(mEditOpenFile, &QLineEdit::textEdited, this, &CFreqDomainOneDerivDlg::onOpenFileTextEdited);

    // 数据文件输出
    connect(mBtnSaveFile, &QPushButton::clicked, this, &CFreqDomainOneDerivDlg::onSaveFileClicked);
    connect(mBtnSaveFileView, &QPushButton::clicked, this, &CFreqDomainOneDerivDlg::onSaveFileViewClicked);
    // 输出文件名编辑框（手工输入时同步成员变量，对应原工程 EN_CHANGE→UpdateData）
    connect(mEditSaveFile, &QLineEdit::textEdited, this, &CFreqDomainOneDerivDlg::onSaveFileTextEdited);

    // 扩边行数/列数微调
    connect(mSpinExRows, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqDomainOneDerivDlg::onExRowsValueChanged);
    connect(mSpinExCols, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqDomainOneDerivDlg::onExColsValueChanged);

    // 扩边方法单选组（id 与 ExpandMethod 枚举一致）
    connect(mButtonGroupExpand, &QButtonGroup::idClicked, this, &CFreqDomainOneDerivDlg::onExpandMethodClicked);

    // 处理参数（角度值微调框）
    connect(mSpinDerivInc, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainOneDerivDlg::onDerivIncValueChanged);
    connect(mSpinDerivDec, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainOneDerivDlg::onDerivDecValueChanged);

    // 确定 / 取消
    connect(mBtnOk, &QPushButton::clicked, this, &CFreqDomainOneDerivDlg::onOkClicked);
    connect(mBtnCancel, &QPushButton::clicked, this, &CFreqDomainOneDerivDlg::onCancelClicked);
...
void CFreqDomainOneDerivDlg::onOpenFileClicked()
{
...
void CFreqDomainOneDerivDlg::onOpenFileTextEdited(const QString& strText)
{
    mInputFilePath = strText;
}

// 功能：读取文件头并刷新网格信息、默认值（对应原工程 ReadData）
void CFreqDomainOneDerivDlg::loadFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
// 功能：生成默认输出文件名（对应原工程 ReadData 中 _HDR_I.grd 命名规则）
void CFreqDomainOneDerivDlg::updateDefaultSavePaths(const QString& strFilePath)
{
    ...
    mOutputFilePath = strBase + QStringLiteral("_HDR_I.grd");
...
// 功能：把网格信息成员变量刷到界面显示
void CFreqDomainOneDerivDlg::updateGridInfoDisplay()
{
...
void CFreqDomainOneDerivDlg::setSpinExtendSize(QSpinBox* pSpin, int nSize)
{
    if (pSpin == NULL)
    {
        return;
...
void CFreqDomainOneDerivDlg::onSaveFileClicked()
{
...
void CFreqDomainOneDerivDlg::onSaveFileTextEdited(const QString& strText)
{
    mOutputFilePath = strText;
}

// 功能：弹出打开文件对话框（.grd 网格数据文件）
QString CFreqDomainOneDerivDlg::askOpenFilePath(const QString& strTitle)
{
...
QString CFreqDomainOneDerivDlg::askSaveFilePath(const QString& strTitle, const QString& strDefaultPath)
{
...
void CFreqDomainOneDerivDlg::viewGridFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
// 功能：显示输入数据文件等值线
void CFreqDomainOneDerivDlg::onOpenFileViewClicked()
{
    viewGridFile(mInputFilePath);
}

// 功能：显示处理结果文件等值线
void CFreqDomainOneDerivDlg::onSaveFileViewClicked()
{
    viewGridFile(mOutputFilePath);
}

// 功能：扩边行数变化（同步成员变量）
void CFreqDomainOneDerivDlg::onExRowsValueChanged(int nValue)
{
    mExRows = nValue;
}

// 功能：扩边列数变化（同步成员变量）
void CFreqDomainOneDerivDlg::onExColsValueChanged(int nValue)
{
    mExCols = nValue;
}

// 功能：数据扩边方法切换（同步成员变量）
void CFreqDomainOneDerivDlg::onExpandMethodClicked(int nMethodId)
{
    mExpandMethod = nMethodId;
}

// 功能：导数倾角变化（同步成员变量）
void CFreqDomainOneDerivDlg::onDerivIncValueChanged(double dValue)
{
    mDerivInc = dValue;
}

// 功能：导数偏角变化（同步成员变量）
void CFreqDomainOneDerivDlg::onDerivDecValueChanged(double dValue)
{
    mDerivDec = dValue;
}

// 功能：确定前校验输入项（对应原工程 OnOK 中的空值检查，并给出更明确的提示）
bool CFreqDomainOneDerivDlg::validateInputs(QString& strError)
{
    // 输入数据文件检查
...
// 功能：组装参数并调用后端 processOneDeriv（对应原工程 OnOK 主体逻辑）
void CFreqDomainOneDerivDlg::runProcess()
{
    // 后端接口按需从全局服务获取（不缓存为成员指针）
    IRgisBackend* pBackend = CBackendService::rgisBackend();
    if (pBackend == NULL)
    ...
    bool bOk = pBackend->processOneDeriv(params, error);
...
void CFreqDomainOneDerivDlg::onOkClicked()
{
    QString strError;
...
void CFreqDomainOneDerivDlg::onCancelClicked()
{
    reject();
}
