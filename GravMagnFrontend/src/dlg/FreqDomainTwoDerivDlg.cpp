// FreqDomainTwoDerivDlg.cpp : 频率域二阶导数对话框实现
//
// 说明：
//   * 界面布局对照 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainTwoDeriv；
//   * 所有回调均为成员函数引用（connect 的第四个参数为 &类名::槽函数），不使用 lambda；
//   * 全程不使用 try/catch，错误一律通过返回值 + 消息框反馈，做足空指针检查；
//   * 中文字符串一律使用 QStringLiteral（源码 UTF-8）。

#include "FreqDomainTwoDerivDlg.h"
...
CFreqDomainTwoDerivDlg::CFreqDomainTwoDerivDlg(QWidget* pParent)
    : QDialog(pParent)
    , mEditOpenFile(NULL)
    , mBtnOpenFile(NULL)
...
CFreqDomainTwoDerivDlg::~CFreqDomainTwoDerivDlg()
{
}

// 功能：按 .rc 布局创建全部控件并连接信号槽
void CFreqDomainTwoDerivDlg::initUi()
{
    setWindowTitle(QStringLiteral("频率域二阶导数"));
    setModal(true);
    ...
    // ================= 处理参数（导数倾角 / 导数偏角）=================
    // 对应 .rc 中的 IDC_DerivInc / IDC_DerivDec（取值范围与原工程 DDV_MinMaxFloat 一致）
    QGroupBox* pGroupParams = new QGroupBox(QStringLiteral("处理参数"), this);
    QGridLayout* pParamsLayout = new QGridLayout(pGroupParams);
    pParamsLayout->addWidget(new QLabel(QStringLiteral("导数倾角"), pGroupParams), 0, 0);
    mSpinDerivInc = new QDoubleSpinBox(pGroupParams);
    mSpinDerivInc->setRange(0.0, 360.0);
    mSpinDerivInc->setDecimals(3);
    mSpinDerivInc->setSingleStep(1.0);
    mSpinDerivInc->setValue(mDerivInc);
    mSpinDerivInc->setAlignment(Qt::AlignCenter);
    mSpinDerivInc->setFixedWidth(110);
    pParamsLayout->addWidget(mSpinDerivInc, 0, 1);
    pParamsLayout->addWidget(new QLabel(QStringLiteral("导数偏角"), pGroupParams), 0, 2);
    mSpinDerivDec = new QDoubleSpinBox(pGroupParams);
    mSpinDerivDec->setRange(0.0, 360.0);
    mSpinDerivDec->setDecimals(3);
    mSpinDerivDec->setSingleStep(1.0);
    mSpinDerivDec->setValue(mDerivDec);
    mSpinDerivDec->setAlignment(Qt::AlignCenter);
    mSpinDerivDec->setFixedWidth(110);
    pParamsLayout->addWidget(mSpinDerivDec, 0, 3);
    ...
    // 输入文件
    connect(mBtnOpenFile, &QPushButton::clicked, this, &CFreqDomainTwoDerivDlg::onOpenFileClicked);
    connect(mBtnOpenFileView, &QPushButton::clicked, this, &CFreqDomainTwoDerivDlg::onOpenFileViewClicked);
    connect(mEditOpenFile, &QLineEdit::textEdited, this, &CFreqDomainTwoDerivDlg::onOpenFileTextEdited);

    // 数据文件输出
    connect(mBtnSaveFile, &QPushButton::clicked, this, &CFreqDomainTwoDerivDlg::onSaveFileClicked);
    connect(mBtnSaveFileView, &QPushButton::clicked, this, &CFreqDomainTwoDerivDlg::onSaveFileViewClicked);
    // 输出文件名编辑框（手工输入时同步成员变量，对应原工程 EN_CHANGE→UpdateData）
    connect(mEditSaveFile, &QLineEdit::textEdited, this, &CFreqDomainTwoDerivDlg::onSaveFileTextEdited);

    // 扩边行数/列数微调
    connect(mSpinExRows, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqDomainTwoDerivDlg::onExRowsValueChanged);
    connect(mSpinExCols, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqDomainTwoDerivDlg::onExColsValueChanged);

    // 扩边方法单选组（id 与 ExpandMethod 枚举一致）
    connect(mButtonGroupExpand, &QButtonGroup::idClicked, this, &CFreqDomainTwoDerivDlg::onExpandMethodClicked);

    // 处理参数（角度值微调框）
    connect(mSpinDerivInc, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainTwoDerivDlg::onDerivIncValueChanged);
    connect(mSpinDerivDec, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainTwoDerivDlg::onDerivDecValueChanged);

    // 确定 / 取消
    connect(mBtnOk, &QPushButton::clicked, this, &CFreqDomainTwoDerivDlg::onOkClicked);
    connect(mBtnCancel, &QPushButton::clicked, this, &CFreqDomainTwoDerivDlg::onCancelClicked);
...
void CFreqDomainTwoDerivDlg::onOpenFileClicked()
{
...
void CFreqDomainTwoDerivDlg::onOpenFileTextEdited(const QString& strText)
{
    mInputFilePath = strText;
}

// 功能：读取文件头并刷新网格信息、默认值（对应原工程 ReadData）
void CFreqDomainTwoDerivDlg::loadFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
// 功能：生成默认输出文件名（对应原工程 ReadData 中 _HDR_II.grd 命名规则）
void CFreqDomainTwoDerivDlg::updateDefaultSavePaths(const QString& strFilePath)
{
    ...
    mOutputFilePath = strBase + QStringLiteral("_HDR_II.grd");
...
// 功能：把网格信息成员变量刷到界面显示
void CFreqDomainTwoDerivDlg::updateGridInfoDisplay()
{
...
void CFreqDomainTwoDerivDlg::setSpinExtendSize(QSpinBox* pSpin, int nSize)
{
    if (pSpin == NULL)
    {
        return;
...
void CFreqDomainTwoDerivDlg::onSaveFileClicked()
{
...
void CFreqDomainTwoDerivDlg::onSaveFileTextEdited(const QString& strText)
{
    mOutputFilePath = strText;
}

// 功能：弹出打开文件对话框（.grd 网格数据文件）
QString CFreqDomainTwoDerivDlg::askOpenFilePath(const QString& strTitle)
{
...
QString CFreqDomainTwoDerivDlg::askSaveFilePath(const QString& strTitle, const QString& strDefaultPath)
{
...
void CFreqDomainTwoDerivDlg::viewGridFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
// 功能：显示输入数据文件等值线
void CFreqDomainTwoDerivDlg::onOpenFileViewClicked()
{
    viewGridFile(mInputFilePath);
}

// 功能：显示处理结果文件等值线
void CFreqDomainTwoDerivDlg::onSaveFileViewClicked()
{
    viewGridFile(mOutputFilePath);
}

// 功能：扩边行数变化（同步成员变量）
void CFreqDomainTwoDerivDlg::onExRowsValueChanged(int nValue)
{
    mExRows = nValue;
}

// 功能：扩边列数变化（同步成员变量）
void CFreqDomainTwoDerivDlg::onExColsValueChanged(int nValue)
{
    mExCols = nValue;
}

// 功能：数据扩边方法切换（同步成员变量）
void CFreqDomainTwoDerivDlg::onExpandMethodClicked(int nMethodId)
{
    mExpandMethod = nMethodId;
}

// 功能：导数倾角变化（同步成员变量）
void CFreqDomainTwoDerivDlg::onDerivIncValueChanged(double dValue)
{
    mDerivInc = dValue;
}

// 功能：导数偏角变化（同步成员变量）
void CFreqDomainTwoDerivDlg::onDerivDecValueChanged(double dValue)
{
    mDerivDec = dValue;
}

// 功能：确定前校验输入项（对应原工程 OnOK 中的空值检查，并给出更明确的提示）
bool CFreqDomainTwoDerivDlg::validateInputs(QString& strError)
{
    // 输入数据文件检查
...
// 功能：组装参数并调用后端 processTwoDeriv（对应原工程 OnOK 主体逻辑）
void CFreqDomainTwoDerivDlg::runProcess()
{
    // 后端接口按需从全局服务获取（不缓存为成员指针）
    IRgisBackend* pBackend = CBackendService::rgisBackend();
    if (pBackend == NULL)
    ...
    params.derivInc = (float)mDerivInc;
    params.derivDec = (float)mDerivDec;

    // 同步调用后端处理（处理期间显示等待光标，与原工程 BeginWaitCursor 一致）
    BackendError error;
    QApplication::setOverrideCursor(Qt::WaitCursor);
    bool bOk = pBackend->processTwoDeriv(params, error);
...
void CFreqDomainTwoDerivDlg::onOkClicked()
{
    QString strError;
...
void CFreqDomainTwoDerivDlg::onCancelClicked()
{
    reject();
}
