// FreqDomainReToPoleDlg.h : 频率域剩余化极（化极）对话框（Qt 前端）
//
// 对应原 MFC 工程：CFreqDomainReToPoleDlg（IDD_FreqDomainReToPole）
// 界面布局依据 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainReToPole 绘制：
//   数据文件输入 / 网格数据信息 / 数据扩边信息 / 数据扩边方法 / 处理参数 / 数据文件输出
//
// 前端职责（本对话框只做界面与交互，算法/数据读写在后端）：
//   1. 输入数据文件的选择、网格信息显示、默认输出文件名生成（_RTP.grd）；
//   2. 扩边行数/列数微调、扩边方法单选（4 种）、测线方向/基线方向/磁化倾角/磁化偏角输入；
//   3. “确定”时校验输入，并调用后端接口 processReToPole 完成处理；
...
// 频率域剩余化极（化极）对话框
class CFreqDomainReToPoleDlg : public QDialog
{
    Q_OBJECT
...
public:
    ...
    explicit CFreqDomainReToPoleDlg(QWidget* pParent = nullptr);

    // 析构函数
    ~CFreqDomainReToPoleDlg() override;
...
private:
    ...
    void runProcess();                          // 功能：组装参数并调用后端 processReToPole
    ...
    QDoubleSpinBox* mSpinDti0;       // 磁化倾角（度，范围 -90~90，默认 90.0）
