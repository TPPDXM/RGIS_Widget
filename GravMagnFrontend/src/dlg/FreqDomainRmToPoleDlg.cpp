// FreqDomainRmToPoleDlg.cpp : 频率域剩余磁化极（剩磁化极）对话框实现
//
// 说明：
//   * 界面布局对照 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainRmToPole；
//   * 原工程此对话框无“数据扩边方法”单选组（扩边由算法内部完成）；
//   * 所有回调均为成员函数引用（connect 的第四个参数为 &类名::槽函数），不使用 lambda；
//   * 全程不使用 try/catch，错误一律通过返回值 + 消息框反馈，做足空指针检查。

#include "FreqDomainRmToPoleDlg.h"
...
CFreqDomainRmToPoleDlg::CFreqDomainRmToPoleDlg(QWidget* pParent)
    : QDialog(pParent)
    , mEditOpenFile(NULL)
    , mBtnOpenFile(NULL)
...
CFreqDomainRmToPoleDlg::~CFreqDomainRmToPoleDlg()
{
}

// 功能：按 .rc 布局创建全部控件并连接信号槽
void CFreqDomainRmToPoleDlg::initUi()
{
    setWindowTitle(QStringLiteral("剩磁化极"));
    setModal(true);
    ...
    // ================= 处理参数（两行两列）=================
    // 对应 .rc 中的 IDC_DTI0 / IDC_DTD0 / IDC_DTI1 / IDC_DTD1
    ...
    // 第一行：地磁倾角 / 地磁偏角
    pParamsLayout->addWidget(new QLabel(QStringLiteral("地磁倾角"), pGroupParams), 0, 0);
    ...
    pParamsLayout->addWidget(mSpinDti0, 0, 1);
    pParamsLayout->addWidget(new QLabel(QStringLiteral("地磁偏角"), pGroupParams), 0, 2);
    ...
    pParamsLayout->addWidget(mSpinDtd0, 0, 3);
    // 第二行：磁化倾角 / 磁化偏角
    pParamsLayout->addWidget(new QLabel(QStringLiteral("磁化倾角"), pGroupParams), 1, 0);
    mSpinDti1 = new QDoubleSpinBox(pGroupParams);
    mSpinDti1->setRange(-90.0, 90.0);
    mSpinDti1->setDecimals(3);
    mSpinDti1->setSingleStep(1.0);
    mSpinDti1->setValue(mDti1);
    mSpinDti1->setAlignment(Qt::AlignCenter);
    mSpinDti1->setFixedWidth(110);
    pParamsLayout->addWidget(mSpinDti1, 1, 1);
    pParamsLayout->addWidget(new QLabel(QStringLiteral("磁化偏角"), pGroupParams), 1, 2);
    mSpinDtd1 = new QDoubleSpinBox(pGroupParams);
    mSpinDtd1->setRange(-360.0, 360.0);
    mSpinDtd1->setDecimals(3);
    mSpinDtd1->setSingleStep(1.0);
    mSpinDtd1->setValue(mDtd1);
    mSpinDtd1->setAlignment(Qt::AlignCenter);
    mSpinDtd1->setFixedWidth(110);
    pParamsLayout->addWidget(mSpinDtd1, 1, 3);
    ...
    // 输入文件
    connect(mBtnOpenFile, &QPushButton::clicked, this, &CFreqDomainRmToPoleDlg::onOpenFileClicked);
    connect(mBtnOpenFileView, &QPushButton::clicked, this, &CFreqDomainRmToPoleDlg::onOpenFileViewClicked);
    connect(mEditOpenFile, &QLineEdit::textEdited, this, &CFreqDomainRmToPoleDlg::onOpenFileTextEdited);

    // 数据文件输出
    connect(mBtnSaveFile, &QPushButton::clicked, this, &CFreqDomainRmToPoleDlg::onSaveFileClicked);
    connect(mBtnSaveFileView, &QPushButton::clicked, this, &CFreqDomainRmToPoleDlg::onSaveFileViewClicked);
    // 输出文件名编辑框（手工输入时同步成员变量，对应原工程 EN_CHANGE→UpdateData）
    connect(mEditSaveFile, &QLineEdit::textEdited, this, &CFreqDomainRmToPoleDlg::onSaveFileTextEdited);

    // 扩边行数/列数微调
    connect(mSpinExRows, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqDomainRmToPoleDlg::onExRowsValueChanged);
    connect(mSpinExCols, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqDomainRmToPoleDlg::onExColsValueChanged);

    // 处理参数（角度值微调框）
    connect(mSpinDti0, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainRmToPoleDlg::onDti0ValueChanged);
    connect(mSpinDtd0, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainRmToPoleDlg::onDtd0ValueChanged);
    connect(mSpinDti1, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainRmToPoleDlg::onDti1ValueChanged);
    connect(mSpinDtd1, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainRmToPoleDlg::onDtd1ValueChanged);

    // 确定 / 取消
    connect(mBtnOk, &QPushButton::clicked, this, &CFreqDomainRmToPoleDlg::onOkClicked);
    connect(mBtnCancel, &QPushButton::clicked, this, &CFreqDomainRmToPoleDlg::onCancelClicked);
...
void CFreqDomainRmToPoleDlg::onOpenFileClicked()
{
...
void CFreqDomainRmToPoleDlg::onOpenFileTextEdited(const QString& strText)
{
    mInputFilePath = strText;
}

// 功能：读取文件头并刷新网格信息、默认值（对应原工程 OnOpenFile 中的 ReadData 逻辑）
void CFreqDomainRmToPoleDlg::loadFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
    ...
    // 默认扩边尺寸（规则见 ExtendGridSize.h，与原工程一致）
...
// 功能：生成默认输出文件名（对应原工程 ReadData 中 _RTP.grd 命名规则）
void CFreqDomainRmToPoleDlg::updateDefaultSavePaths(const QString& strFilePath)
{
...
// 功能：把网格信息成员变量刷到界面显示
void CFreqDomainRmToPoleDlg::updateGridInfoDisplay()
{
...
void CFreqDomainRmToPoleDlg::setSpinExtendSize(QSpinBox* pSpin, int nSize)
{
    if (pSpin == NULL)
    {
        return;
...
void CFreqDomainRmToPoleDlg::onSaveFileClicked()
{
...
void CFreqDomainRmToPoleDlg::onSaveFileTextEdited(const QString& strText)
{
    mOutputFilePath = strText;
}

// 功能：弹出打开文件对话框（.grd 网格数据文件）
QString CFreqDomainRmToPoleDlg::askOpenFilePath(const QString& strTitle)
{
...
QString CFreqDomainRmToPoleDlg::askSaveFilePath(const QString& strTitle, const QString& strDefaultPath)
{
...
void CFreqDomainRmToPoleDlg::viewGridFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
// 功能：显示输入数据文件等值线
void CFreqDomainRmToPoleDlg::onOpenFileViewClicked()
{
    viewGridFile(mInputFilePath);
}

// 功能：显示处理结果文件等值线
void CFreqDomainRmToPoleDlg::onSaveFileViewClicked()
{
    viewGridFile(mOutputFilePath);
}

// 功能：扩边行数变化（同步成员变量）
void CFreqDomainRmToPoleDlg::onExRowsValueChanged(int nValue)
{
    mExRows = nValue;
}

// 功能：扩边列数变化（同步成员变量）
void CFreqDomainRmToPoleDlg::onExColsValueChanged(int nValue)
{
    mExCols = nValue;
}

// 功能：地磁倾角变化（同步成员变量）
void CFreqDomainRmToPoleDlg::onDti0ValueChanged(double dValue)
{
    mDti0 = dValue;
}

// 功能：地磁偏角变化（同步成员变量）
void CFreqDomainRmToPoleDlg::onDtd0ValueChanged(double dValue)
{
    mDtd0 = dValue;
}

// 功能：磁化倾角变化（同步成员变量）
void CFreqDomainRmToPoleDlg::onDti1ValueChanged(double dValue)
{
    mDti1 = dValue;
}

// 功能：磁化偏角变化（同步成员变量）
void CFreqDomainRmToPoleDlg::onDtd1ValueChanged(double dValue)
{
    mDtd1 = dValue;
}

// 功能：确定前校验输入项（对应原工程 OnOK 中的空值检查，并给出更明确的提示）
bool CFreqDomainRmToPoleDlg::validateInputs(QString& strError)
{
    // 输入数据文件检查
...
// 功能：组装参数并调用后端 processRmToPole（对应原工程 OnOK 主体逻辑）
void CFreqDomainRmToPoleDlg::runProcess()
{
    // 后端接口按需从全局服务获取（不缓存为成员指针）
    IRgisBackend* pBackend = CBackendService::rgisBackend();
    if (pBackend == NULL)
    ...
    params.dti1 = (float)mDti1;
    params.dtd1 = (float)mDtd1;

    // 同步调用后端处理（处理期间显示等待光标，与原工程 BeginWaitCursor 一致）
    BackendError error;
    QApplication::setOverrideCursor(Qt::WaitCursor);
    bool bOk = pBackend->processRmToPole(params, error);
...
void CFreqDomainRmToPoleDlg::onOkClicked()
{
    QString strError;
...
void CFreqDomainRmToPoleDlg::onCancelClicked()
{
    reject();
}
