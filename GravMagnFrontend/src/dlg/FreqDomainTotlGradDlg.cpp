// FreqDomainTotlGradDlg.cpp : 频率域总梯度（解析信号）对话框实现
//
// 说明：
//   * 界面布局对照 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainTotlGrad；
//   * 所有回调均为成员函数引用（connect 的第四个参数为 &类名::槽函数），不使用 lambda；
//   * 全程不使用 try/catch，错误一律通过返回值 + 消息框反馈，做足空指针检查；
//   * 中文字符串一律用 QStringLiteral 构造。

#include "FreqDomainTotlGradDlg.h"
...
CFreqDomainTotlGradDlg::CFreqDomainTotlGradDlg(QWidget* pParent)
    : QDialog(pParent)
    , mEditOpenFile(NULL)
    , mBtnOpenFile(NULL)
...
CFreqDomainTotlGradDlg::~CFreqDomainTotlGradDlg()
{
}

// 功能：按 .rc 布局创建全部控件并连接信号槽
void CFreqDomainTotlGradDlg::initUi()
{
    setWindowTitle(QStringLiteral("解析信号"));
    setModal(true);
    ...
    // 输入文件
    connect(mBtnOpenFile, &QPushButton::clicked, this, &CFreqDomainTotlGradDlg::onOpenFileClicked);
    connect(mBtnOpenFileView, &QPushButton::clicked, this, &CFreqDomainTotlGradDlg::onOpenFileViewClicked);
    connect(mEditOpenFile, &QLineEdit::textEdited, this, &CFreqDomainTotlGradDlg::onOpenFileTextEdited);

    // 数据文件输出
    connect(mBtnSaveFile, &QPushButton::clicked, this, &CFreqDomainTotlGradDlg::onSaveFileClicked);
    connect(mBtnSaveFileView, &QPushButton::clicked, this, &CFreqDomainTotlGradDlg::onSaveFileViewClicked);
    // 输出文件名编辑框（手工输入时同步成员变量，对应原工程 EN_CHANGE→UpdateData）
    connect(mEditSaveFile, &QLineEdit::textEdited, this, &CFreqDomainTotlGradDlg::onSaveFileTextEdited);

    // 扩边行数/列数微调
    connect(mSpinExRows, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqDomainTotlGradDlg::onExRowsValueChanged);
    connect(mSpinExCols, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqDomainTotlGradDlg::onExColsValueChanged);

    // 扩边方法单选组（id 与 ExpandMethod 枚举一致）
    connect(mButtonGroupExpand, &QButtonGroup::idClicked, this, &CFreqDomainTotlGradDlg::onExpandMethodClicked);

    // 确定 / 取消
    connect(mBtnOk, &QPushButton::clicked, this, &CFreqDomainTotlGradDlg::onOkClicked);
    connect(mBtnCancel, &QPushButton::clicked, this, &CFreqDomainTotlGradDlg::onCancelClicked);
...
void CFreqDomainTotlGradDlg::onOpenFileClicked()
{
...
void CFreqDomainTotlGradDlg::onOpenFileTextEdited(const QString& strText)
{
    mInputFilePath = strText;
}

// 功能：读取文件头并刷新网格信息、默认值（对应原工程 ReadData）
void CFreqDomainTotlGradDlg::loadFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
// 功能：生成默认输出文件名（对应原工程 ReadData 中 _AS.grd 命名规则）
void CFreqDomainTotlGradDlg::updateDefaultSavePaths(const QString& strFilePath)
{
    ...
    mOutputFilePath = strBase + QStringLiteral("_AS.grd");
...
// 功能：把网格信息成员变量刷到界面显示
void CFreqDomainTotlGradDlg::updateGridInfoDisplay()
{
...
void CFreqDomainTotlGradDlg::setSpinExtendSize(QSpinBox* pSpin, int nSize)
{
    if (pSpin == NULL)
    {
        return;
...
void CFreqDomainTotlGradDlg::onSaveFileClicked()
{
...
void CFreqDomainTotlGradDlg::onSaveFileTextEdited(const QString& strText)
{
    mOutputFilePath = strText;
}

// 功能：弹出打开文件对话框（.grd 网格数据文件）
QString CFreqDomainTotlGradDlg::askOpenFilePath(const QString& strTitle)
{
...
QString CFreqDomainTotlGradDlg::askSaveFilePath(const QString& strTitle, const QString& strDefaultPath)
{
...
void CFreqDomainTotlGradDlg::viewGridFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
// 功能：显示输入数据文件等值线
void CFreqDomainTotlGradDlg::onOpenFileViewClicked()
{
    viewGridFile(mInputFilePath);
}

// 功能：显示处理结果文件等值线
void CFreqDomainTotlGradDlg::onSaveFileViewClicked()
{
    viewGridFile(mOutputFilePath);
}

// 功能：扩边行数变化（同步成员变量）
void CFreqDomainTotlGradDlg::onExRowsValueChanged(int nValue)
{
    mExRows = nValue;
}

// 功能：扩边列数变化（同步成员变量）
void CFreqDomainTotlGradDlg::onExColsValueChanged(int nValue)
{
    mExCols = nValue;
}

// 功能：数据扩边方法切换（同步成员变量）
void CFreqDomainTotlGradDlg::onExpandMethodClicked(int nMethodId)
{
    mExpandMethod = nMethodId;
}

// 功能：确定前校验输入项（对应原工程 OnOK 中的空值检查，并给出更明确的提示）
bool CFreqDomainTotlGradDlg::validateInputs(QString& strError)
{
    // 输入数据文件检查
...
// 功能：组装参数并调用后端 processTotlGrad（对应原工程 OnOK 主体逻辑）
void CFreqDomainTotlGradDlg::runProcess()
{
    // 后端接口按需从全局服务获取（不缓存为成员指针）
    IRgisBackend* pBackend = CBackendService::rgisBackend();
    if (pBackend == NULL)
    ...
    bool bOk = pBackend->processTotlGrad(params, error);
...
void CFreqDomainTotlGradDlg::onOkClicked()
{
    QString strError;
...
void CFreqDomainTotlGradDlg::onCancelClicked()
{
    reject();
}
