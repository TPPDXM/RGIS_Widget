// FreqDomainNormFilterDlg.h : 频率域正则化滤波对话框（Qt 前端）
//
// 对应原 MFC 工程：CFreqDomainNormFilterDlg（IDD_FreqDomainNormFilter）
// 界面布局依据 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainNormFilter 绘制：
//   数据文件输入 / 网格数据信息 / 数据扩边信息 / 数据扩边方法 / 处理参数 /
//   区域场数据文件输出 / 剩余场数据文件输出
//
// 前端职责（本对话框只做界面与交互，算法/数据读写在后端）：
//   1. 输入数据文件的选择、网格信息显示、默认输出文件名生成（_Reg.grd / _Res.grd）；
//   2. 扩边行数/列数（2 的幂）微调、扩边方法单选（4 种）、水平几何尺度输入；
//   3. “确定”时校验输入，并调用后端接口 processNormFilter 完成处理；
...
public:
    ...
    explicit CFreqDomainNormFilterDlg(QWidget* pParent = nullptr);

    // 析构函数
    ~CFreqDomainNormFilterDlg() override;
...
private slots:
    ...
    void onSaveFileClicked();               // 功能：“...”选择区域场结果数据文件
    void onSaveFileViewClicked();           // 功能：显示区域场结果文件等值线
    void onSaveFileTextEdited(const QString& strText);  // 功能：手动输入区域场文件名（同步成员变量）
    void onSaveResFileClicked();            // 功能：“...”选择剩余场结果数据文件
    void onSaveResFileViewClicked();        // 功能：显示剩余场结果文件等值线
    void onSaveResFileTextEdited(const QString& strText);  // 功能：手动输入剩余场文件名（同步成员变量）
    ...
    // ===== 处理参数回调 =====
    void onScaleOfRowValueChanged(int nValue);  // 功能：水平几何尺度变化（同步成员变量）
...
private:
    ...
    // ===== 界面控件（处理参数，对应 .rc 中 IDC_ScaleOfRow）=====
    QSpinBox*       mSpinScaleOfRow; // 水平几何尺度（整数，范围 1~999999，默认 10）

    // ===== 界面控件（区域场 / 剩余场数据文件输出）=====
    QLineEdit*  mEditSaveFile;       // 区域场结果数据文件名（默认 基准名 + "_Reg.grd"）
    QLineEdit*  mEditSaveResFile;    // 剩余场结果数据文件名（默认 基准名 + "_Res.grd"）
    QPushButton* mBtnSaveFile;       // 区域场输出文件“...”按钮
    QPushButton* mBtnSaveResFile;    // 剩余场输出文件“...”按钮
    QPushButton* mBtnSaveFileView;   // 区域场输出文件“显示”按钮
    QPushButton* mBtnSaveResFileView;// 剩余场输出文件“显示”按钮
    ...
    QString mRegionalFilePath;       // 区域场输出文件路径（对应原 m_strSaveFile）
    QString mResidualFilePath;       // 剩余场输出文件路径（对应原 m_strSaveResFile）
