// FreqDomainUpwardDlg.cpp : 频率域向上延拓对话框实现
//
// 说明：
//   * 界面布局对照 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainUpward；
//   * 所有回调均为成员函数引用（connect 的第四个参数为 &类名::槽函数），不使用 lambda；
//   * 全程不使用 try/catch，错误一律通过返回值 + 消息框反馈，做足空指针检查；
//   * 中文字符串一律使用 QStringLiteral（源码 UTF-8）。

#include "FreqDomainUpwardDlg.h"
...
CFreqDomainUpwardDlg::CFreqDomainUpwardDlg(QWidget* pParent)
    : QDialog(pParent)
    , mEditOpenFile(NULL)
    , mBtnOpenFile(NULL)
...
CFreqDomainUpwardDlg::~CFreqDomainUpwardDlg()
{
}

// 功能：按 .rc 布局创建全部控件并连接信号槽
void CFreqDomainUpwardDlg::initUi()
{
    setWindowTitle(QStringLiteral("频率域向上延拓"));
    setModal(true);
    ...
    // ================= 处理参数（向上延拓高度）=================
    // 对应 .rc 中的 IDC_UpHeight（取值范围与原工程 DDV_MinMaxFloat 一致）
    ...
    mSpinHeight->setRange(0.0, 1.0e7);
    ...
    // 输入文件
    connect(mBtnOpenFile, &QPushButton::clicked, this, &CFreqDomainUpwardDlg::onOpenFileClicked);
    connect(mBtnOpenFileView, &QPushButton::clicked, this, &CFreqDomainUpwardDlg::onOpenFileViewClicked);
    connect(mEditOpenFile, &QLineEdit::textEdited, this, &CFreqDomainUpwardDlg::onOpenFileTextEdited);

    // 数据文件输出
    connect(mBtnSaveFile, &QPushButton::clicked, this, &CFreqDomainUpwardDlg::onSaveFileClicked);
    connect(mBtnSaveFileView, &QPushButton::clicked, this, &CFreqDomainUpwardDlg::onSaveFileViewClicked);
    // 输出文件名编辑框（手工输入时同步成员变量，对应原工程 EN_CHANGE→UpdateData）
    connect(mEditSaveFile, &QLineEdit::textEdited, this, &CFreqDomainUpwardDlg::onSaveFileTextEdited);

    // 扩边行数/列数微调
    connect(mSpinExRows, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqDomainUpwardDlg::onExRowsValueChanged);
    connect(mSpinExCols, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqDomainUpwardDlg::onExColsValueChanged);

    // 扩边方法单选组（id 与 ExpandMethod 枚举一致）
    connect(mButtonGroupExpand, &QButtonGroup::idClicked, this, &CFreqDomainUpwardDlg::onExpandMethodClicked);

    // 处理参数（向上延拓高度）
    connect(mSpinHeight, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainUpwardDlg::onHeightValueChanged);

    // 确定 / 取消
    connect(mBtnOk, &QPushButton::clicked, this, &CFreqDomainUpwardDlg::onOkClicked);
    connect(mBtnCancel, &QPushButton::clicked, this, &CFreqDomainUpwardDlg::onCancelClicked);
...
void CFreqDomainUpwardDlg::onOpenFileClicked()
{
...
void CFreqDomainUpwardDlg::onOpenFileTextEdited(const QString& strText)
{
    mInputFilePath = strText;
}

// 功能：读取文件头并刷新网格信息、默认值（对应原工程 ReadData）
void CFreqDomainUpwardDlg::loadFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
    ...
    // 默认向上延拓高度：2 倍列距（与原工程 ReadData 中 "%.3f", 2 * xStep 一致）
...
void CFreqDomainUpwardDlg::updateDefaultSavePaths(const QString& strFilePath)
{
...
// 功能：把网格信息成员变量刷到界面显示
void CFreqDomainUpwardDlg::updateGridInfoDisplay()
{
...
void CFreqDomainUpwardDlg::setSpinExtendSize(QSpinBox* pSpin, int nSize)
{
    if (pSpin == NULL)
    {
        return;
...
void CFreqDomainUpwardDlg::onSaveFileClicked()
{
...
void CFreqDomainUpwardDlg::onSaveFileTextEdited(const QString& strText)
{
    mOutputFilePath = strText;
}

// 功能：弹出打开文件对话框（.grd 网格数据文件）
QString CFreqDomainUpwardDlg::askOpenFilePath(const QString& strTitle)
{
...
QString CFreqDomainUpwardDlg::askSaveFilePath(const QString& strTitle, const QString& strDefaultPath)
{
...
void CFreqDomainUpwardDlg::viewGridFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
// 功能：显示输入数据文件等值线
void CFreqDomainUpwardDlg::onOpenFileViewClicked()
{
    viewGridFile(mInputFilePath);
}

// 功能：显示处理结果文件等值线
void CFreqDomainUpwardDlg::onSaveFileViewClicked()
{
    viewGridFile(mOutputFilePath);
}

// 功能：扩边行数变化（同步成员变量）
void CFreqDomainUpwardDlg::onExRowsValueChanged(int nValue)
{
    mExRows = nValue;
}

// 功能：扩边列数变化（同步成员变量）
void CFreqDomainUpwardDlg::onExColsValueChanged(int nValue)
{
    mExCols = nValue;
}

// 功能：数据扩边方法切换（同步成员变量）
void CFreqDomainUpwardDlg::onExpandMethodClicked(int nMethodId)
{
    mExpandMethod = nMethodId;
}

// 功能：向上延拓高度变化（同步成员变量）
void CFreqDomainUpwardDlg::onHeightValueChanged(double dValue)
{
    mHeight = dValue;
}

// 功能：确定前校验输入项（对应原工程 OnOK 中的空值检查，并给出更明确的提示）
bool CFreqDomainUpwardDlg::validateInputs(QString& strError)
{
    // 输入数据文件检查
...
// 功能：组装参数并调用后端 processUpward（对应原工程 OnOK 主体逻辑）
void CFreqDomainUpwardDlg::runProcess()
{
    // 后端接口按需从全局服务获取（不缓存为成员指针）
    IRgisBackend* pBackend = CBackendService::rgisBackend();
    if (pBackend == NULL)
    ...
    bool bOk = pBackend->processUpward(params, error);
...
void CFreqDomainUpwardDlg::onOkClicked()
{
    QString strError;
...
void CFreqDomainUpwardDlg::onCancelClicked()
{
    reject();
}
