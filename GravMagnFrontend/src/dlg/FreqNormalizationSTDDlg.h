// FreqNormalizationSTDDlg.h : 频率域归一化标准差（归一化标准方差）对话框（Qt 前端）
//
// 对应原 MFC 工程：CFreqNormalizationSTDDlg（IDD_FreqNormalizationSTD）
// 界面布局依据 RGISGravMagnDataProcessing.rc 中 IDD_FreqNormalizationSTD 绘制：
//   网格数据文件输入 / 网格数据信息 / 处理参数 / 网格数据文件输出
// 说明：原工程此对话框无“数据扩边信息/数据扩边方法”组（扩边由算法内部完成）；
//       算法经外部动态库 PfProcesses.dll（iComType = 503）完成，后端实现时可桥接或等价重写。
//
// 前端职责（本对话框只做界面与交互，算法/数据读写在后端）：
//   1. 输入数据文件的选择、网格信息显示、默认输出文件名生成（_NSTD.grd）；
//   2. 窗口行数/窗口列数/正则化因子输入；“确定”前校验网格行列距相等；
//   3. “确定”时校验输入，并调用后端接口 processNormalizationSTD 完成处理；
...
// 频率域归一化标准差对话框
class CFreqNormalizationSTDDlg : public QDialog
{
    Q_OBJECT
...
public:
    ...
    explicit CFreqNormalizationSTDDlg(QWidget* pParent = nullptr);

    // 析构函数
    ~CFreqNormalizationSTDDlg() override;
...
private:
    ...
    void runProcess();                          // 功能：组装参数并调用后端 processNormalizationSTD
    ...
    // ===== 界面控件（数据文件输出）=====
    QLineEdit*  mEditSaveFile;       // 处理结果数据文件名（默认 基准名 + "_NSTD.grd"）
