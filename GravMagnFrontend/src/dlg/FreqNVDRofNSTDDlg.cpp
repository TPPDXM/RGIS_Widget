// FreqNVDRofNSTDDlg.cpp : 频率域归一化标准差的垂直导数比（NVDR-of-NSTD）对话框实现
//
// 说明：
//   * 界面布局对照 RGISGravMagnDataProcessing.rc 中 IDD_FreqNVDRofNSTD；
//   * 所有回调均为成员函数引用（connect 的第四个参数为 &类名::槽函数），不使用 lambda；
//   * 全程不使用 try/catch，错误一律通过返回值 + 消息框反馈，做足空指针检查；
//   * 中文字符串一律使用 QStringLiteral（源码 UTF-8）。

#include "FreqNVDRofNSTDDlg.h"
...
CFreqNVDRofNSTDDlg::CFreqNVDRofNSTDDlg(QWidget* pParent)
    : QDialog(pParent)
    , mEditOpenFile(NULL)
    , mBtnOpenFile(NULL)
...
CFreqNVDRofNSTDDlg::~CFreqNVDRofNSTDDlg()
{
}

// 功能：按 .rc 布局创建全部控件并连接信号槽
void CFreqNVDRofNSTDDlg::initUi()
{
    setWindowTitle(QStringLiteral("归一化标准差垂向导数"));
    setModal(true);
    ...
    // 输入文件
    connect(mBtnOpenFile, &QPushButton::clicked, this, &CFreqNVDRofNSTDDlg::onOpenFileClicked);
    connect(mBtnOpenFileView, &QPushButton::clicked, this, &CFreqNVDRofNSTDDlg::onOpenFileViewClicked);
    connect(mEditOpenFile, &QLineEdit::textEdited, this, &CFreqNVDRofNSTDDlg::onOpenFileTextEdited);

    // 数据文件输出
    connect(mBtnSaveFile, &QPushButton::clicked, this, &CFreqNVDRofNSTDDlg::onSaveFileClicked);
    connect(mBtnSaveFileView, &QPushButton::clicked, this, &CFreqNVDRofNSTDDlg::onSaveFileViewClicked);
    // 输出文件名编辑框（手工输入时同步成员变量，对应原工程 EN_CHANGE→UpdateData）
    connect(mEditSaveFile, &QLineEdit::textEdited, this, &CFreqNVDRofNSTDDlg::onSaveFileTextEdited);

    // 处理参数
    connect(mSpinWinRows, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqNVDRofNSTDDlg::onWinRowsValueChanged);
    connect(mSpinWinCols, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqNVDRofNSTDDlg::onWinColsValueChanged);
    connect(mSpinAlfa, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqNVDRofNSTDDlg::onAlfaValueChanged);

    // 确定 / 取消
    connect(mBtnOk, &QPushButton::clicked, this, &CFreqNVDRofNSTDDlg::onOkClicked);
    connect(mBtnCancel, &QPushButton::clicked, this, &CFreqNVDRofNSTDDlg::onCancelClicked);
...
void CFreqNVDRofNSTDDlg::onOpenFileClicked()
{
...
void CFreqNVDRofNSTDDlg::onOpenFileTextEdited(const QString& strText)
{
    mInputFilePath = strText;
}

// 功能：读取文件头并刷新网格信息、默认值（对应原工程 ReadData）
void CFreqNVDRofNSTDDlg::loadFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
// 功能：生成默认输出文件名（对应原工程 ReadData 中 _NVDR_NSTD.grd 命名规则）
void CFreqNVDRofNSTDDlg::updateDefaultSavePaths(const QString& strFilePath)
{
    ...
    mOutputFilePath = strBase + QStringLiteral("_NVDR_NSTD.grd");
...
// 功能：把网格信息成员变量刷到界面显示
void CFreqNVDRofNSTDDlg::updateGridInfoDisplay()
{
...
void CFreqNVDRofNSTDDlg::onSaveFileClicked()
{
...
void CFreqNVDRofNSTDDlg::onSaveFileTextEdited(const QString& strText)
{
    mOutputFilePath = strText;
}

// 功能：弹出打开文件对话框（.grd 网格数据文件）
QString CFreqNVDRofNSTDDlg::askOpenFilePath(const QString& strTitle)
{
...
QString CFreqNVDRofNSTDDlg::askSaveFilePath(const QString& strTitle, const QString& strDefaultPath)
{
...
void CFreqNVDRofNSTDDlg::viewGridFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
// 功能：显示输入数据文件等值线
void CFreqNVDRofNSTDDlg::onOpenFileViewClicked()
{
    viewGridFile(mInputFilePath);
}

// 功能：显示处理结果文件等值线
void CFreqNVDRofNSTDDlg::onSaveFileViewClicked()
{
    viewGridFile(mOutputFilePath);
}

// 功能：窗口行数变化（同步成员变量）
void CFreqNVDRofNSTDDlg::onWinRowsValueChanged(int nValue)
{
    mWinRows = nValue;
}

// 功能：窗口列数变化（同步成员变量）
void CFreqNVDRofNSTDDlg::onWinColsValueChanged(int nValue)
{
    mWinCols = nValue;
}

// 功能：正则化因子变化（同步成员变量）
void CFreqNVDRofNSTDDlg::onAlfaValueChanged(double dValue)
{
    mAlfa = dValue;
}

// 功能：确定前校验输入项（对应原工程 OnOK 中的空值检查与行列距校验）
bool CFreqNVDRofNSTDDlg::validateInputs(QString& strError)
{
    // 输入数据文件检查
...
// 功能：组装参数并调用后端 processNVDRofNSTD（对应原工程 OnOK 主体逻辑）
void CFreqNVDRofNSTDDlg::runProcess()
{
    // 后端接口按需从全局服务获取（不缓存为成员指针）
    IRgisBackend* pBackend = CBackendService::rgisBackend();
    if (pBackend == NULL)
    ...
    bool bOk = pBackend->processNVDRofNSTD(params, error);
    ...
    if (bOk)
    {
        ...
        QMessageBox::information(this, QStringLiteral("处理完成"),
            QStringLiteral("归一化标准方差垂向导数计算结束!"));
    }
    else
    {
...
void CFreqNVDRofNSTDDlg::onOkClicked()
{
    QString strError;
...
void CFreqNVDRofNSTDDlg::onCancelClicked()
{
    reject();
}
