// FreqDomainMaxiGradDlg.h : 频率域最大梯度（最大水平方向导数）对话框（Qt 前端）
//
// 对应原 MFC 工程：CFreqDomainMaxiGradDlg（IDD_FreqDomainMaxiGrad）
// 界面布局依据 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainMaxiGrad 绘制（2 列窄对话框）：
//   数据文件输入 / 网格数据信息（2 行：行数、列数 / 行距、列距）/
//   数据扩边信息 / 数据扩边方法（仅 3 种）/ 数据文件输出
//
// 前端职责（本对话框只做界面与交互，算法/数据读写在后端）：
//   1. 输入数据文件的选择（手工输入 + 浏览）、网格信息显示、
//      默认输出文件名生成（基准名 + "_Pro.grd"）；
//   2. 扩边行数/列数（2 的幂）微调、扩边方法单选（3 种，无最小曲率）；
//   3. “确定”时校验输入，并调用后端接口 processMaxiGrad 完成处理
//      （方向角步长 5 度、36 个方向为原工程硬编码，界面无输入）；
...
// 频率域最大梯度（最大水平方向导数）对话框
class CFreqDomainMaxiGradDlg : public QDialog
{
    Q_OBJECT
...
public:
    ...
    explicit CFreqDomainMaxiGradDlg(QWidget* pParent = nullptr);

    // 析构函数
    ~CFreqDomainMaxiGradDlg() override;
...
private:
    ...
    void updateDefaultSavePaths(const QString& strFilePath);// 功能：生成默认输出文件名（基准名 + "_Pro.grd"）
    ...
    void runProcess();                          // 功能：组装参数并调用后端 processMaxiGrad
    ...
    int     mExpandMethod;  // 数据扩边方法（对应 ExpandMethod 枚举，原单选按钮组，仅 3 种）
