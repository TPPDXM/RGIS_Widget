// FreqMaximumDerivativeDlg.cpp : 频率域最大导数（最大水平方向导数）对话框实现
//
// 说明：
//   * 界面布局对照 RGISGravMagnDataProcessing.rc 中 IDD_FreqMaximumDerivative；
//   * 所有回调均为成员函数引用（connect 的第四个参数为 &类名::槽函数），不使用 lambda；
//   * 全程不使用 try/catch，错误一律通过返回值 + 消息框反馈，做足空指针检查；
//   * 中文字符串一律使用 QStringLiteral（源码 UTF-8）。

#include "FreqMaximumDerivativeDlg.h"
...
CFreqMaximumDerivativeDlg::CFreqMaximumDerivativeDlg(QWidget* pParent)
    : QDialog(pParent)
    , mEditOpenFile(NULL)
    , mBtnOpenFile(NULL)
...
CFreqMaximumDerivativeDlg::~CFreqMaximumDerivativeDlg()
{
}

// 功能：按 .rc 布局创建全部控件并连接信号槽
void CFreqMaximumDerivativeDlg::initUi()
{
    ...
    // ================= 处理参数（水平方向角度间隔）=================
    // 对应 .rc 中的 IDC_AzimuthStep（取值范围与原工程 DDV_MinMaxInt 一致）
    QGroupBox* pGroupParams = new QGroupBox(QStringLiteral("处理参数"), this);
    QGridLayout* pParamsLayout = new QGridLayout(pGroupParams);
    pParamsLayout->addWidget(new QLabel(QStringLiteral("水平方向角度间隔"), pGroupParams), 0, 0);
    mSpinAzimuthStep = new QSpinBox(pGroupParams);
    mSpinAzimuthStep->setRange(1, 20);
    mSpinAzimuthStep->setValue(mAzimuthStep);
    mSpinAzimuthStep->setAlignment(Qt::AlignCenter);
    mSpinAzimuthStep->setFixedWidth(100);
    pParamsLayout->addWidget(mSpinAzimuthStep, 0, 1);
    ...
    // 输入文件
    connect(mBtnOpenFile, &QPushButton::clicked, this, &CFreqMaximumDerivativeDlg::onOpenFileClicked);
    connect(mBtnOpenFileView, &QPushButton::clicked, this, &CFreqMaximumDerivativeDlg::onOpenFileViewClicked);
    connect(mEditOpenFile, &QLineEdit::textEdited, this, &CFreqMaximumDerivativeDlg::onOpenFileTextEdited);

    // 数据文件输出
    connect(mBtnSaveFile, &QPushButton::clicked, this, &CFreqMaximumDerivativeDlg::onSaveFileClicked);
    connect(mBtnSaveFileView, &QPushButton::clicked, this, &CFreqMaximumDerivativeDlg::onSaveFileViewClicked);
    // 输出文件名编辑框（手工输入时同步成员变量，对应原工程 EN_CHANGE→UpdateData）
    connect(mEditSaveFile, &QLineEdit::textEdited, this, &CFreqMaximumDerivativeDlg::onSaveFileTextEdited);

    // 处理参数（水平方向角度间隔）
    connect(mSpinAzimuthStep, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqMaximumDerivativeDlg::onAzimuthStepValueChanged);

    // 确定 / 取消
    connect(mBtnOk, &QPushButton::clicked, this, &CFreqMaximumDerivativeDlg::onOkClicked);
    connect(mBtnCancel, &QPushButton::clicked, this, &CFreqMaximumDerivativeDlg::onCancelClicked);
...
void CFreqMaximumDerivativeDlg::onOpenFileClicked()
{
...
void CFreqMaximumDerivativeDlg::onOpenFileTextEdited(const QString& strText)
{
    mInputFilePath = strText;
}

// 功能：读取文件头并刷新网格信息、默认值（对应原工程 ReadData）
void CFreqMaximumDerivativeDlg::loadFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
// 功能：生成默认输出文件名（对应原工程 ReadData 中 _MaxHDR.grd 命名规则）
void CFreqMaximumDerivativeDlg::updateDefaultSavePaths(const QString& strFilePath)
{
    ...
    mOutputFilePath = strBase + QStringLiteral("_MaxHDR.grd");
...
// 功能：把网格信息成员变量刷到界面显示
void CFreqMaximumDerivativeDlg::updateGridInfoDisplay()
{
...
void CFreqMaximumDerivativeDlg::onSaveFileClicked()
{
...
void CFreqMaximumDerivativeDlg::onSaveFileTextEdited(const QString& strText)
{
    mOutputFilePath = strText;
}

// 功能：弹出打开文件对话框（.grd 网格数据文件）
QString CFreqMaximumDerivativeDlg::askOpenFilePath(const QString& strTitle)
{
...
QString CFreqMaximumDerivativeDlg::askSaveFilePath(const QString& strTitle, const QString& strDefaultPath)
{
...
void CFreqMaximumDerivativeDlg::viewGridFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
// 功能：显示输入数据文件等值线
void CFreqMaximumDerivativeDlg::onOpenFileViewClicked()
{
    viewGridFile(mInputFilePath);
}

// 功能：显示处理结果文件等值线
void CFreqMaximumDerivativeDlg::onSaveFileViewClicked()
{
    viewGridFile(mOutputFilePath);
}

// 功能：水平方向角度间隔变化（同步成员变量）
void CFreqMaximumDerivativeDlg::onAzimuthStepValueChanged(int nValue)
{
    mAzimuthStep = nValue;
}

// 功能：确定前校验输入项（对应原工程 OnOK 中的空值检查，并给出更明确的提示）
bool CFreqMaximumDerivativeDlg::validateInputs(QString& strError)
{
    // 输入数据文件检查
...
// 功能：组装参数并调用后端 processMaximumDerivative（对应原工程 OnOK 主体逻辑）
void CFreqMaximumDerivativeDlg::runProcess()
{
    // 后端接口按需从全局服务获取（不缓存为成员指针）
    IRgisBackend* pBackend = CBackendService::rgisBackend();
    if (pBackend == NULL)
...
void CFreqMaximumDerivativeDlg::onOkClicked()
{
    QString strError;
...
void CFreqMaximumDerivativeDlg::onCancelClicked()
{
    reject();
}
