// FreqDomainMultiCpnDlg.h : 频率域任意方向分量转换（多分量）对话框（Qt 前端）
//
// 对应原 MFC 工程：CFreqDomainMultiCpnDlg（IDD_FreqDomainMultiCpn）
// 界面布局依据 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainMultiCpn 绘制：
...
//      测线方向/基线方向/磁化倾角/磁化偏角/转换倾角/转换偏角输入；
//   3. “确定”时校验输入，并调用后端接口 processMultiCpn 完成处理；
...
// 频率域任意方向分量转换（多分量）对话框
class CFreqDomainMultiCpnDlg : public QDialog
{
    Q_OBJECT
...
public:
    ...
    explicit CFreqDomainMultiCpnDlg(QWidget* pParent = nullptr);

    // 析构函数
    ~CFreqDomainMultiCpnDlg() override;
...
private slots:
    ...
    void onTransIncValueChanged(double dValue); // 功能：转换倾角变化（同步成员变量）
    void onTransDecValueChanged(double dValue); // 功能：转换偏角变化（同步成员变量）
...
private:
    ...
    void runProcess();                          // 功能：组装参数并调用后端 processMultiCpn
    ...
    //           IDC_DTI0 / IDC_DTD0 / IDC_TransInc / IDC_TransDec）=====
    ...
    QDoubleSpinBox* mSpinTransInc;   // 转换倾角（度，范围 0~180，默认 90.0）
    QDoubleSpinBox* mSpinTransDec;   // 转换偏角（度，范围 -360~360，默认 0.0）

    // ===== 界面控件（数据文件输出）=====
    QLineEdit*  mEditSaveFile;       // 处理结果数据文件名（默认 基准名 + "_Pro.grd"）
    ...
    double  mTransInc;      // 转换倾角（原 m_fTransInc，默认 90.0）
    double  mTransDec;      // 转换偏角（原 m_fTransDec，默认 0.0）
