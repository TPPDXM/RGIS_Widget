// FreqDomainGradwardDlg.h : 频率域逐次向下延拓（正则化滤波）对话框（Qt 前端）
//
// 对应原 MFC 工程：CFreqDomainGradwardDlg（IDD_FreqDomainGradward）
// 界面布局依据 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainGradward 绘制：
//   数据文件输入 / 网格数据信息 / 数据扩边信息 / 数据扩边方法 /
//   延拓参数（延拓高度、延拓次数、逐次延拓参数表格）/ 数据文件输出
//
// 前端职责（本对话框只做界面与交互，算法/数据读写在后端）：
//   1. 输入数据文件的选择、网格信息显示、默认输出文件名生成（基准名 + "OutN.grd"）；
//   2. 扩边行数/列数（2 的幂）微调、扩边方法单选、延拓高度（默认 10 倍列距）与延拓次数；
//   3. 逐次延拓参数表格：序号 / 延拓高度 / 水平几何尺度（对应原工程 MSFlexGrid）；
//   4. “确定”时校验输入，并调用后端接口 processGradward 完成处理；
...
public:
    ...
    explicit CFreqDomainGradwardDlg(QWidget* pParent = nullptr);

    // 析构函数
    ~CFreqDomainGradwardDlg() override;
...
private slots:
    ...
    void onTimesValueChanged(int nValue);       // 功能：延拓次数变化（重建逐次延拓参数表格默认值）
    void onTableClicked();                      // 功能：点击逐次延拓参数表格（行内容为空时填入默认值）
...
private:
    ...
    // ===== 逐次延拓参数表格 =====
    void rebuildStepTable();                    // 功能：按延拓次数重建表格行并填入默认值（对应原工程 OnClickMsFlexGrid）
    bool readStepTable(QString& strError);      // 功能：读取表格各步参数并校验（对应原工程 OnOK 中的表格校验）
    ...
    // ===== 界面控件（延拓参数）=====
    QDoubleSpinBox* mSpinHeight;     // 总延拓高度（浮点，范围 0~999999，读文件后默认 10 倍列距）
    QSpinBox*       mSpinTimes;      // 延拓次数（整数，范围 1~9999，默认 5）
    QTableWidget*   mTableSteps;     // 逐次延拓参数表格（3 列：序号 / 延拓高度 / 水平几何尺度，对应原 MSFlexGrid）

    // ===== 界面控件（数据文件输出）=====
    QLineEdit*  mEditSaveFile;       // 处理结果数据文件名（默认 基准名 + "OutN.grd"）
    ...
    // ===== 逐次延拓参数（从表格读取后的暂存）=====
    // 单个延拓步的暂存参数（对应后端接口 GradwardStepParams）
    struct SStepValue
    {
        double  mHeight;    // 该步延拓高度增量
        int     mScale;     // 水平几何尺度（正则化滤波参数）
    ...
    std::vector<SStepValue> mStepValues;    // 各步参数暂存（长度 = 延拓次数）
    ...
    double  mHeight;        // 总延拓高度（原 m_fHeight）
    int     mTimes;         // 延拓次数（原 m_nTimes）
