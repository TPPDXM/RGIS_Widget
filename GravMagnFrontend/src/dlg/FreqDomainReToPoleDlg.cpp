// FreqDomainReToPoleDlg.cpp : 频率域剩余化极（化极）对话框实现
//
// 说明：
//   * 界面布局对照 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainReToPole；
//   * 所有回调均为成员函数引用（connect 的第四个参数为 &类名::槽函数），不使用 lambda；
//   * 全程不使用 try/catch，错误一律通过返回值 + 消息框反馈，做足空指针检查。

#include "FreqDomainReToPoleDlg.h"
...
CFreqDomainReToPoleDlg::CFreqDomainReToPoleDlg(QWidget* pParent)
    : QDialog(pParent)
    , mEditOpenFile(NULL)
    , mBtnOpenFile(NULL)
...
CFreqDomainReToPoleDlg::~CFreqDomainReToPoleDlg()
{
}

// 功能：按 .rc 布局创建全部控件并连接信号槽
void CFreqDomainReToPoleDlg::initUi()
{
    setWindowTitle(QStringLiteral("化极"));
    setModal(true);
    ...
    // 输入文件
    connect(mBtnOpenFile, &QPushButton::clicked, this, &CFreqDomainReToPoleDlg::onOpenFileClicked);
    connect(mBtnOpenFileView, &QPushButton::clicked, this, &CFreqDomainReToPoleDlg::onOpenFileViewClicked);
    connect(mEditOpenFile, &QLineEdit::textEdited, this, &CFreqDomainReToPoleDlg::onOpenFileTextEdited);

    // 数据文件输出
    connect(mBtnSaveFile, &QPushButton::clicked, this, &CFreqDomainReToPoleDlg::onSaveFileClicked);
    connect(mBtnSaveFileView, &QPushButton::clicked, this, &CFreqDomainReToPoleDlg::onSaveFileViewClicked);
    // 输出文件名编辑框（手工输入时同步成员变量，对应原工程 EN_CHANGE→UpdateData）
    connect(mEditSaveFile, &QLineEdit::textEdited, this, &CFreqDomainReToPoleDlg::onSaveFileTextEdited);

    // 扩边行数/列数微调
    connect(mSpinExRows, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqDomainReToPoleDlg::onExRowsValueChanged);
    connect(mSpinExCols, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqDomainReToPoleDlg::onExColsValueChanged);

    // 扩边方法单选组（id 与 ExpandMethod 枚举一致）
    connect(mButtonGroupExpand, &QButtonGroup::idClicked, this, &CFreqDomainReToPoleDlg::onExpandMethodClicked);

    // 处理参数（角度值微调框）
    connect(mSpinProfile, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainReToPoleDlg::onProfileValueChanged);
    connect(mSpinBaseLine, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainReToPoleDlg::onBaseLineValueChanged);
    connect(mSpinDti0, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainReToPoleDlg::onDti0ValueChanged);
    connect(mSpinDtd0, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainReToPoleDlg::onDtd0ValueChanged);

    // 确定 / 取消
    connect(mBtnOk, &QPushButton::clicked, this, &CFreqDomainReToPoleDlg::onOkClicked);
    connect(mBtnCancel, &QPushButton::clicked, this, &CFreqDomainReToPoleDlg::onCancelClicked);
...
void CFreqDomainReToPoleDlg::onOpenFileClicked()
{
...
void CFreqDomainReToPoleDlg::onOpenFileTextEdited(const QString& strText)
{
    mInputFilePath = strText;
}

// 功能：读取文件头并刷新网格信息、默认值（对应原工程 ReadData）
void CFreqDomainReToPoleDlg::loadFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
void CFreqDomainReToPoleDlg::updateDefaultSavePaths(const QString& strFilePath)
{
...
// 功能：把网格信息成员变量刷到界面显示
void CFreqDomainReToPoleDlg::updateGridInfoDisplay()
{
...
void CFreqDomainReToPoleDlg::setSpinExtendSize(QSpinBox* pSpin, int nSize)
{
    if (pSpin == NULL)
    {
        return;
...
void CFreqDomainReToPoleDlg::onSaveFileClicked()
{
...
void CFreqDomainReToPoleDlg::onSaveFileTextEdited(const QString& strText)
{
    mOutputFilePath = strText;
}

// 功能：弹出打开文件对话框（.grd 网格数据文件）
QString CFreqDomainReToPoleDlg::askOpenFilePath(const QString& strTitle)
{
...
QString CFreqDomainReToPoleDlg::askSaveFilePath(const QString& strTitle, const QString& strDefaultPath)
{
...
void CFreqDomainReToPoleDlg::viewGridFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
// 功能：显示输入数据文件等值线
void CFreqDomainReToPoleDlg::onOpenFileViewClicked()
{
    viewGridFile(mInputFilePath);
}

// 功能：显示处理结果文件等值线
void CFreqDomainReToPoleDlg::onSaveFileViewClicked()
{
    viewGridFile(mOutputFilePath);
}

// 功能：扩边行数变化（同步成员变量）
void CFreqDomainReToPoleDlg::onExRowsValueChanged(int nValue)
{
    mExRows = nValue;
}

// 功能：扩边列数变化（同步成员变量）
void CFreqDomainReToPoleDlg::onExColsValueChanged(int nValue)
{
    mExCols = nValue;
}

// 功能：数据扩边方法切换（同步成员变量）
void CFreqDomainReToPoleDlg::onExpandMethodClicked(int nMethodId)
{
    mExpandMethod = nMethodId;
}

// 功能：测线方向变化（同步成员变量）
void CFreqDomainReToPoleDlg::onProfileValueChanged(double dValue)
{
    mProfile = dValue;
}

// 功能：基线方向变化（同步成员变量）
void CFreqDomainReToPoleDlg::onBaseLineValueChanged(double dValue)
{
    mBaseLine = dValue;
}

// 功能：磁化倾角变化（同步成员变量）
void CFreqDomainReToPoleDlg::onDti0ValueChanged(double dValue)
{
    mDti0 = dValue;
}

// 功能：磁化偏角变化（同步成员变量）
void CFreqDomainReToPoleDlg::onDtd0ValueChanged(double dValue)
{
    mDtd0 = dValue;
}

// 功能：确定前校验输入项（对应原工程 OnOK 中的空值检查，并给出更明确的提示）
bool CFreqDomainReToPoleDlg::validateInputs(QString& strError)
{
    // 输入数据文件检查
...
// 功能：组装参数并调用后端 processReToPole（对应原工程 OnOK 主体逻辑）
void CFreqDomainReToPoleDlg::runProcess()
{
    // 后端接口按需从全局服务获取（不缓存为成员指针）
    IRgisBackend* pBackend = CBackendService::rgisBackend();
    if (pBackend == NULL)
    ...
    bool bOk = pBackend->processReToPole(params, error);
...
void CFreqDomainReToPoleDlg::onOkClicked()
{
    QString strError;
...
void CFreqDomainReToPoleDlg::onCancelClicked()
{
    reject();
}
