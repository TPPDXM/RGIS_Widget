// FreqDomainUpwardDlg.h : 频率域向上延拓对话框（Qt 前端）
//
// 对应原 MFC 工程：CFreqDomainUpwardDlg（IDD_FreqDomainUpward）
// 界面布局依据 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainUpward 绘制：
//   数据文件输入 / 网格数据信息 / 数据扩边信息 / 数据扩边方法 / 处理参数 / 数据文件输出
//
// 前端职责（本对话框只做界面与交互，算法/数据读写在后端）：
//   1. 输入数据文件的选择、网格信息显示、默认输出文件名生成（_Pro.grd）；
//   2. 扩边行数/列数（2 的幂）微调、扩边方法单选（4 种）、向上延拓高度输入
//      （读文件后默认 2 倍列距）；
//   3. “确定”时校验输入，并调用后端接口 processUpward 完成处理；
...
// 频率域向上延拓对话框
class CFreqDomainUpwardDlg : public QDialog
{
    Q_OBJECT
...
public:
    ...
    explicit CFreqDomainUpwardDlg(QWidget* pParent = nullptr);

    // 析构函数
    ~CFreqDomainUpwardDlg() override;
...
private slots:
    ...
    // ===== 处理参数回调 =====
    void onHeightValueChanged(double dValue);   // 功能：向上延拓高度变化（同步成员变量）
...
private:
    ...
    void runProcess();                          // 功能：组装参数并调用后端 processUpward
    ...
    // ===== 界面控件（处理参数，对应 .rc 中 IDC_UpHeight）=====
    QDoubleSpinBox* mSpinHeight;     // 向上延拓高度（度/米，范围 0~1e7，读文件后默认 2 倍列距）
    ...
    double  mHeight;        // 向上延拓高度（原 m_fHeight，读文件后默认 2 倍列距）
};
