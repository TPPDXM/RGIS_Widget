// FreqDomainCmpsFilterDlg.h : 频率域组合滤波（补偿圆滑滤波）对话框（Qt 前端）
//
// 对应原 MFC 工程：CFreqDomainCmpsFilterDlg（IDD_FreqDomainCmpsFilter）
// 界面布局依据 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainCmpsFilter 绘制：
//   数据文件输入 / 网格数据信息 / 数据扩边信息 / 数据扩边方法 / 处理参数 /
//   区域场数据文件输出 / 剩余场数据文件输出
//
// 前端职责（本对话框只做界面与交互，算法/数据读写在后端）：
//   1. 输入数据文件的选择、网格信息显示、默认输出文件名生成；
//   2. 扩边行数/列数（2 的幂）的微调、扩边方法单选、处理参数输入；
//   3. “确定”时校验输入，并调用后端接口 processCmpsFilter 完成处理；
...
public:
    ...
    explicit CFreqDomainCmpsFilterDlg(const QString& strFileNames, QWidget* pParent = nullptr);

    // 析构函数
    ~CFreqDomainCmpsFilterDlg() override;
...
private:
    ...
    void runProcess();                          // 功能：组装参数并调用后端 processCmpsFilter
    ...
    // ===== 界面控件（区域场 / 剩余场数据文件输出）=====
    QLineEdit*  mEditSaveFile;       // 区域场结果数据文件名
    QLineEdit*  mEditSaveResFile;    // 剩余场结果数据文件名
