// FreqDomainPsudoGraDlg.h : 频率域伪重力（磁源重力异常）对话框（Qt 前端）
//
// 对应原 MFC 工程：CFreqDomainPsudoGraDlg（IDD_FreqDomainPsudoGra）
// 界面布局依据 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainPsudoGra 绘制：
//   数据文件输入 / 网格数据信息 / 数据扩边信息 / 数据扩边方法 /
//   处理参数（磁化强度为 A/m，剩余密度为 g/cm^3）/ 数据文件输出
//
// 前端职责（本对话框只做界面与交互，算法/数据读写在后端）：
//   1. 输入数据文件的选择、网格信息显示、默认输出文件名生成（_Psudo.grd）；
//   2. 扩边行数/列数（2 的幂）微调、扩边方法单选（4 种）、
//      测线方向/基线方向/磁化倾角/磁化偏角/磁化强度/剩余密度/衰减因子输入；
//   3. “确定”时校验输入，并调用后端接口 processPsudoGra 完成处理；
...
// 频率域伪重力（磁源重力异常）对话框
class CFreqDomainPsudoGraDlg : public QDialog
{
    Q_OBJECT
...
public:
    ...
    explicit CFreqDomainPsudoGraDlg(QWidget* pParent = nullptr);

    // 析构函数
    ~CFreqDomainPsudoGraDlg() override;
...
private slots:
    ...
    void onMagnetizationValueChanged(double dValue); // 功能：磁化强度变化（同步成员变量）
    void onDensityValueChanged(double dValue);  // 功能：剩余密度变化（同步成员变量）
    void onCosineValueChanged(int nValue);      // 功能：衰减因子变化（同步成员变量）
...
private:
    ...
    void runProcess();                          // 功能：组装参数并调用后端 processPsudoGra
    ...
    //           IDC_DTI0 / IDC_DTD0 / IDC_Magnetization / IDC_Density / IDC_Cosine）=====
    ...
    QDoubleSpinBox* mSpinMagnetization;     // 磁化强度（A/m，范围 0~1e12，默认 1.0）
    QDoubleSpinBox* mSpinDensity;           // 剩余密度（g/cm^3，范围 -5~5，默认 0.1）
    QSpinBox*       mSpinCosine;            // 衰减因子（范围 1~20，默认 1）

    // ===== 界面控件（数据文件输出）=====
    QLineEdit*  mEditSaveFile;       // 处理结果数据文件名（默认 基准名 + "_Psudo.grd"）
    ...
    double  mMagnetization; // 磁化强度（原 m_fMagnetization，默认 1.0）
    double  mDensity;       // 剩余密度（原 m_fDensity，默认 0.1）
    int     mCosine;        // 衰减因子（原 m_nCosine，默认 1）
};
