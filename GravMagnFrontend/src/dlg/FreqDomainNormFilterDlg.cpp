// FreqDomainNormFilterDlg.cpp : 频率域正则化滤波对话框实现
//
// 说明：
//   * 界面布局对照 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainNormFilter；
//   * 所有回调均为成员函数引用（connect 的第四个参数为 &类名::槽函数），不使用 lambda；
//   * 全程不使用 try/catch，错误一律通过返回值 + 消息框反馈，做足空指针检查。

#include "FreqDomainNormFilterDlg.h"
...
CFreqDomainNormFilterDlg::CFreqDomainNormFilterDlg(QWidget* pParent)
    : QDialog(pParent)
    , mEditOpenFile(NULL)
    , mBtnOpenFile(NULL)
...
CFreqDomainNormFilterDlg::~CFreqDomainNormFilterDlg()
{
}
...
void CFreqDomainNormFilterDlg::initUi()
{
    ...
    mSpinScaleOfRow->setAlignment(Qt::AlignCenter);
    mSpinScaleOfRow->setFixedWidth(120);
    pParamsLayout->addWidget(mSpinScaleOfRow, 0, 1);
    ...
    // 输入文件
    connect(mBtnOpenFile, &QPushButton::clicked, this, &CFreqDomainNormFilterDlg::onOpenFileClicked);
    connect(mBtnOpenFileView, &QPushButton::clicked, this, &CFreqDomainNormFilterDlg::onOpenFileViewClicked);
    connect(mEditOpenFile, &QLineEdit::textEdited, this, &CFreqDomainNormFilterDlg::onOpenFileTextEdited);

    // 区域场 / 剩余场输出
    connect(mBtnSaveFile, &QPushButton::clicked, this, &CFreqDomainNormFilterDlg::onSaveFileClicked);
    connect(mBtnSaveFileView, &QPushButton::clicked, this, &CFreqDomainNormFilterDlg::onSaveFileViewClicked);
    connect(mBtnSaveResFile, &QPushButton::clicked, this, &CFreqDomainNormFilterDlg::onSaveResFileClicked);
    connect(mBtnSaveResFileView, &QPushButton::clicked, this, &CFreqDomainNormFilterDlg::onSaveResFileViewClicked);
    ...
    connect(mButtonGroupExpand, &QButtonGroup::idClicked, this, &CFreqDomainNormFilterDlg::onExpandMethodClicked);

    // 处理参数（水平几何尺度）
    connect(mSpinScaleOfRow, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqDomainNormFilterDlg::onScaleOfRowValueChanged);

    // 确定 / 取消
    connect(mBtnOk, &QPushButton::clicked, this, &CFreqDomainNormFilterDlg::onOkClicked);
    connect(mBtnCancel, &QPushButton::clicked, this, &CFreqDomainNormFilterDlg::onCancelClicked);
...
void CFreqDomainNormFilterDlg::onOpenFileTextEdited(const QString& strText)
{
    mInputFilePath = strText;
}

// 功能：读取文件头并刷新网格信息、默认值（对应原工程 ReadData）
void CFreqDomainNormFilterDlg::loadFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
void CFreqDomainNormFilterDlg::updateDefaultSavePaths(const QString& strFilePath)
{
...
// 功能：把网格信息成员变量刷到界面显示
void CFreqDomainNormFilterDlg::updateGridInfoDisplay()
{
...
void CFreqDomainNormFilterDlg::onSaveFileTextEdited(const QString& strText)
{
    mRegionalFilePath = strText;
}

// 功能：手动输入剩余场文件名（同步成员变量，对应原工程 OnChangeSaveResFile）
void CFreqDomainNormFilterDlg::onSaveResFileTextEdited(const QString& strText)
{
...
void CFreqDomainNormFilterDlg::viewGridFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
// 功能：显示输入数据文件等值线
void CFreqDomainNormFilterDlg::onOpenFileViewClicked()
{
    viewGridFile(mInputFilePath);
}

// 功能：显示区域场结果文件等值线
void CFreqDomainNormFilterDlg::onSaveFileViewClicked()
{
    viewGridFile(mRegionalFilePath);
}

// 功能：显示剩余场结果文件等值线
void CFreqDomainNormFilterDlg::onSaveResFileViewClicked()
{
    viewGridFile(mResidualFilePath);
}

// 功能：扩边行数变化（同步成员变量）
void CFreqDomainNormFilterDlg::onExRowsValueChanged(int nValue)
{
    mExRows = nValue;
}

// 功能：扩边列数变化（同步成员变量）
void CFreqDomainNormFilterDlg::onExColsValueChanged(int nValue)
{
    mExCols = nValue;
}

// 功能：数据扩边方法切换（同步成员变量）
void CFreqDomainNormFilterDlg::onExpandMethodClicked(int nMethodId)
{
    mExpandMethod = nMethodId;
}

// 功能：水平几何尺度变化（同步成员变量）
void CFreqDomainNormFilterDlg::onScaleOfRowValueChanged(int nValue)
{
    mScaleOfRow = nValue;
}

// 功能：确定前校验输入项（对应原工程 OnOK 中的空值检查，并给出更明确的提示）
bool CFreqDomainNormFilterDlg::validateInputs(QString& strError)
{
    // 输入数据文件检查
...
// 功能：组装参数并调用后端 processNormFilter（对应原工程 OnOK 主体逻辑）
void CFreqDomainNormFilterDlg::runProcess()
{
    // 后端接口按需从全局服务获取（不缓存为成员指针）
    IRgisBackend* pBackend = CBackendService::rgisBackend();
    if (pBackend == NULL)
    ...
    params.scaleOfRow = mScaleOfRow;

    // 同步调用后端处理（处理期间显示等待光标，与原工程 BeginWaitCursor 一致）
    BackendError error;
    QApplication::setOverrideCursor(Qt::WaitCursor);
    bool bOk = pBackend->processNormFilter(params, error);
...
void CFreqDomainNormFilterDlg::onOkClicked()
{
    QString strError;
...
void CFreqDomainNormFilterDlg::onCancelClicked()
{
    reject();
}
