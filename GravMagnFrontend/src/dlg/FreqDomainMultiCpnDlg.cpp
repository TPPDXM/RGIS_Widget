// FreqDomainMultiCpnDlg.cpp : 频率域任意方向分量转换（多分量）对话框实现
//
// 说明：
//   * 界面布局对照 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainMultiCpn；
//   * 所有回调均为成员函数引用（connect 的第四个参数为 &类名::槽函数），不使用 lambda；
//   * 全程不使用 try/catch，错误一律通过返回值 + 消息框反馈，做足空指针检查。

#include "FreqDomainMultiCpnDlg.h"
...
CFreqDomainMultiCpnDlg::CFreqDomainMultiCpnDlg(QWidget* pParent)
    : QDialog(pParent)
    , mEditOpenFile(NULL)
    , mBtnOpenFile(NULL)
...
CFreqDomainMultiCpnDlg::~CFreqDomainMultiCpnDlg()
{
}

// 功能：按 .rc 布局创建全部控件并连接信号槽
void CFreqDomainMultiCpnDlg::initUi()
{
    setWindowTitle(QString::fromUtf8("任意磁化方向分量转换"));
    setModal(true);
    ...
    //           IDC_TransInc / IDC_TransDec（取值范围与原工程 DDV_MinMaxFloat 一致）
    QGroupBox* pGroupParams = new QGroupBox(QString::fromUtf8("处理参数"), this);
    QGridLayout* pParamsLayout = new QGridLayout(pGroupParams);
    // 第一行：测线方向 / 基线方向
    pParamsLayout->addWidget(new QLabel(QString::fromUtf8("测线方向"), pGroupParams), 0, 0);
    ...
    pParamsLayout->addWidget(new QLabel(QString::fromUtf8("基线方向"), pGroupParams), 0, 2);
    ...
    // 第二行：磁化倾角 / 磁化偏角 / 转换倾角 / 转换偏角
    pParamsLayout->addWidget(new QLabel(QString::fromUtf8("磁化倾角"), pGroupParams), 1, 0);
    ...
    pParamsLayout->addWidget(mSpinDti0, 1, 1);
    pParamsLayout->addWidget(new QLabel(QString::fromUtf8("磁化偏角"), pGroupParams), 1, 2);
    ...
    pParamsLayout->addWidget(mSpinDtd0, 1, 3);
    pParamsLayout->addWidget(new QLabel(QString::fromUtf8("转换倾角"), pGroupParams), 1, 4);
    mSpinTransInc = new QDoubleSpinBox(pGroupParams);
    mSpinTransInc->setRange(0.0, 180.0);
    mSpinTransInc->setDecimals(3);
    mSpinTransInc->setSingleStep(1.0);
    mSpinTransInc->setValue(mTransInc);
    mSpinTransInc->setAlignment(Qt::AlignCenter);
    mSpinTransInc->setFixedWidth(110);
    pParamsLayout->addWidget(mSpinTransInc, 1, 5);
    pParamsLayout->addWidget(new QLabel(QString::fromUtf8("转换偏角"), pGroupParams), 1, 6);
    mSpinTransDec = new QDoubleSpinBox(pGroupParams);
    mSpinTransDec->setRange(-360.0, 360.0);
    mSpinTransDec->setDecimals(3);
    mSpinTransDec->setSingleStep(1.0);
    mSpinTransDec->setValue(mTransDec);
    mSpinTransDec->setAlignment(Qt::AlignCenter);
    mSpinTransDec->setFixedWidth(110);
    pParamsLayout->addWidget(mSpinTransDec, 1, 7);
    ...
    // 输入文件
    connect(mBtnOpenFile, &QPushButton::clicked, this, &CFreqDomainMultiCpnDlg::onOpenFileClicked);
    connect(mBtnOpenFileView, &QPushButton::clicked, this, &CFreqDomainMultiCpnDlg::onOpenFileViewClicked);
    connect(mEditOpenFile, &QLineEdit::textEdited, this, &CFreqDomainMultiCpnDlg::onOpenFileTextEdited);

    // 数据文件输出
    connect(mBtnSaveFile, &QPushButton::clicked, this, &CFreqDomainMultiCpnDlg::onSaveFileClicked);
    connect(mBtnSaveFileView, &QPushButton::clicked, this, &CFreqDomainMultiCpnDlg::onSaveFileViewClicked);
    // 输出文件名编辑框（手工输入时同步成员变量，对应原工程 EN_CHANGE→UpdateData）
    connect(mEditSaveFile, &QLineEdit::textEdited, this, &CFreqDomainMultiCpnDlg::onSaveFileTextEdited);

    // 扩边行数/列数（2 的幂微调）
    connect(mSpinExRows, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqDomainMultiCpnDlg::onExRowsValueChanged);
    connect(mSpinExCols, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqDomainMultiCpnDlg::onExColsValueChanged);

    // 扩边方法单选组（id 与 ExpandMethod 枚举一致）
    connect(mButtonGroupExpand, &QButtonGroup::idClicked, this, &CFreqDomainMultiCpnDlg::onExpandMethodClicked);

    // 处理参数（角度值微调框）
    connect(mSpinProfile, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainMultiCpnDlg::onProfileValueChanged);
    connect(mSpinBaseLine, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainMultiCpnDlg::onBaseLineValueChanged);
    connect(mSpinDti0, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainMultiCpnDlg::onDti0ValueChanged);
    connect(mSpinDtd0, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainMultiCpnDlg::onDtd0ValueChanged);
    connect(mSpinTransInc, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainMultiCpnDlg::onTransIncValueChanged);
    connect(mSpinTransDec, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainMultiCpnDlg::onTransDecValueChanged);

    // 确定 / 取消
    connect(mBtnOk, &QPushButton::clicked, this, &CFreqDomainMultiCpnDlg::onOkClicked);
    connect(mBtnCancel, &QPushButton::clicked, this, &CFreqDomainMultiCpnDlg::onCancelClicked);
...
void CFreqDomainMultiCpnDlg::onOpenFileClicked()
{
...
void CFreqDomainMultiCpnDlg::onOpenFileTextEdited(const QString& strText)
{
    mInputFilePath = strText;
}

// 功能：读取文件头并刷新网格信息、默认值（对应原工程 ReadData）
void CFreqDomainMultiCpnDlg::loadFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
void CFreqDomainMultiCpnDlg::updateDefaultSavePaths(const QString& strFilePath)
{
...
// 功能：把网格信息成员变量刷到界面显示
void CFreqDomainMultiCpnDlg::updateGridInfoDisplay()
{
...
void CFreqDomainMultiCpnDlg::setSpinExtendSize(CPow2SpinBox* pSpin, int nSize)
{
    if (pSpin == NULL)
    {
        return;
...
void CFreqDomainMultiCpnDlg::onSaveFileClicked()
{
...
void CFreqDomainMultiCpnDlg::onSaveFileTextEdited(const QString& strText)
{
    mOutputFilePath = strText;
}

// 功能：弹出打开文件对话框（.grd 网格数据文件）
QString CFreqDomainMultiCpnDlg::askOpenFilePath(const QString& strTitle)
{
...
QString CFreqDomainMultiCpnDlg::askSaveFilePath(const QString& strTitle, const QString& strDefaultPath)
{
...
void CFreqDomainMultiCpnDlg::viewGridFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
// 功能：显示输入数据文件等值线
void CFreqDomainMultiCpnDlg::onOpenFileViewClicked()
{
    viewGridFile(mInputFilePath);
}

// 功能：显示处理结果文件等值线
void CFreqDomainMultiCpnDlg::onSaveFileViewClicked()
{
    viewGridFile(mOutputFilePath);
}

// 功能：扩边行数变化（同步成员变量）
void CFreqDomainMultiCpnDlg::onExRowsValueChanged(int nValue)
{
    mExRows = nValue;
}

// 功能：扩边列数变化（同步成员变量）
void CFreqDomainMultiCpnDlg::onExColsValueChanged(int nValue)
{
    mExCols = nValue;
}

// 功能：数据扩边方法切换（同步成员变量）
void CFreqDomainMultiCpnDlg::onExpandMethodClicked(int nMethodId)
{
    mExpandMethod = nMethodId;
}

// 功能：测线方向变化（同步成员变量）
void CFreqDomainMultiCpnDlg::onProfileValueChanged(double dValue)
{
    mProfile = dValue;
}

// 功能：基线方向变化（同步成员变量）
void CFreqDomainMultiCpnDlg::onBaseLineValueChanged(double dValue)
{
    mBaseLine = dValue;
}

// 功能：磁化倾角变化（同步成员变量）
void CFreqDomainMultiCpnDlg::onDti0ValueChanged(double dValue)
{
    mDti0 = dValue;
}

// 功能：磁化偏角变化（同步成员变量）
void CFreqDomainMultiCpnDlg::onDtd0ValueChanged(double dValue)
{
    mDtd0 = dValue;
}

// 功能：转换倾角变化（同步成员变量）
void CFreqDomainMultiCpnDlg::onTransIncValueChanged(double dValue)
{
    mTransInc = dValue;
}

// 功能：转换偏角变化（同步成员变量）
void CFreqDomainMultiCpnDlg::onTransDecValueChanged(double dValue)
{
    mTransDec = dValue;
}

// 功能：确定前校验输入项（对应原工程 OnOK 中的空值检查，并给出更明确的提示）
bool CFreqDomainMultiCpnDlg::validateInputs(QString& strError)
{
    // 输入数据文件检查
...
// 功能：组装参数并调用后端 processMultiCpn（对应原工程 OnOK 主体逻辑）
void CFreqDomainMultiCpnDlg::runProcess()
{
    if (mBackend == NULL)
    {
    ...
    params.transInc = (float)mTransInc;
    params.transDec = (float)mTransDec;
    ...
    bool bOk = mBackend->processMultiCpn(params, error);
    ...
    if (bOk)
    {
        ...
        QMessageBox::information(this, QString::fromUtf8("处理完成"),
            QString::fromUtf8("频率域任意方向分量转换结束!"));
    }
    else
    {
...
void CFreqDomainMultiCpnDlg::onOkClicked()
{
    QString strError;
...
void CFreqDomainMultiCpnDlg::onCancelClicked()
{
    reject();
}
