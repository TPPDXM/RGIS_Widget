// FreqDomainDzToPoleDlg.cpp : 频率域 ΔZ 化极（Za 化极）对话框实现
//
// 说明：
//   * 界面布局对照 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainDzToPole；
//   * 所有回调均为成员函数引用（connect 的第四个参数为 &类名::槽函数），不使用 lambda；
//   * 全程不使用 try/catch，错误一律通过返回值 + 消息框反馈，做足空指针检查；
//   * 中文字符串一律使用 QStringLiteral（源码 UTF-8）。

#include "FreqDomainDzToPoleDlg.h"
...
CFreqDomainDzToPoleDlg::CFreqDomainDzToPoleDlg(QWidget* pParent)
    : QDialog(pParent)
    , mEditOpenFile(NULL)
    , mBtnOpenFile(NULL)
...
CFreqDomainDzToPoleDlg::~CFreqDomainDzToPoleDlg()
{
}

// 功能：按 .rc 布局创建全部控件并连接信号槽
void CFreqDomainDzToPoleDlg::initUi()
{
    setWindowTitle(QStringLiteral("Za 化极"));
    setModal(true);
    ...
    // ================= 处理参数（磁化偏角 / 磁化倾角）=================
    // 对应 .rc 中的 IDC_DTD0 / IDC_DTI0（取值范围与原工程 DDV_MinMaxFloat 一致）
    QGroupBox* pGroupParams = new QGroupBox(QStringLiteral("处理参数"), this);
    QGridLayout* pParamsLayout = new QGridLayout(pGroupParams);
    pParamsLayout->addWidget(new QLabel(QStringLiteral("磁化倾角"), pGroupParams), 0, 0);
    ...
    pParamsLayout->addWidget(new QLabel(QStringLiteral("磁化偏角"), pGroupParams), 0, 2);
    ...
    // 输入文件
    connect(mBtnOpenFile, &QPushButton::clicked, this, &CFreqDomainDzToPoleDlg::onOpenFileClicked);
    connect(mBtnOpenFileView, &QPushButton::clicked, this, &CFreqDomainDzToPoleDlg::onOpenFileViewClicked);
    connect(mEditOpenFile, &QLineEdit::textEdited, this, &CFreqDomainDzToPoleDlg::onOpenFileTextEdited);

    // 数据文件输出
    connect(mBtnSaveFile, &QPushButton::clicked, this, &CFreqDomainDzToPoleDlg::onSaveFileClicked);
    connect(mBtnSaveFileView, &QPushButton::clicked, this, &CFreqDomainDzToPoleDlg::onSaveFileViewClicked);
    // 输出文件名编辑框（手工输入时同步成员变量，对应原工程 EN_CHANGE→UpdateData）
    connect(mEditSaveFile, &QLineEdit::textEdited, this, &CFreqDomainDzToPoleDlg::onSaveFileTextEdited);

    // 扩边行数/列数微调
    connect(mSpinExRows, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqDomainDzToPoleDlg::onExRowsValueChanged);
    connect(mSpinExCols, QOverload<int>::of(&QSpinBox::valueChanged), this, &CFreqDomainDzToPoleDlg::onExColsValueChanged);

    // 处理参数（角度值微调框）
    connect(mSpinDti0, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainDzToPoleDlg::onDti0ValueChanged);
    connect(mSpinDtd0, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &CFreqDomainDzToPoleDlg::onDtd0ValueChanged);

    // 确定 / 取消
    connect(mBtnOk, &QPushButton::clicked, this, &CFreqDomainDzToPoleDlg::onOkClicked);
    connect(mBtnCancel, &QPushButton::clicked, this, &CFreqDomainDzToPoleDlg::onCancelClicked);
...
void CFreqDomainDzToPoleDlg::onOpenFileClicked()
{
...
void CFreqDomainDzToPoleDlg::onOpenFileTextEdited(const QString& strText)
{
    mInputFilePath = strText;
}

// 功能：读取文件头并刷新网格信息、默认值（对应原工程 ReadData）
void CFreqDomainDzToPoleDlg::loadFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
    ...
    // 默认扩边尺寸（规则见 FrontendUtils.h 的 suggestExtendSize，与原工程一致）
...
void CFreqDomainDzToPoleDlg::updateDefaultSavePaths(const QString& strFilePath)
{
...
// 功能：把网格信息成员变量刷到界面显示
void CFreqDomainDzToPoleDlg::updateGridInfoDisplay()
{
...
void CFreqDomainDzToPoleDlg::setSpinExtendSize(QSpinBox* pSpin, int nSize)
{
    if (pSpin == NULL)
    {
        return;
...
void CFreqDomainDzToPoleDlg::onSaveFileClicked()
{
...
void CFreqDomainDzToPoleDlg::onSaveFileTextEdited(const QString& strText)
{
    mOutputFilePath = strText;
}

// 功能：弹出打开文件对话框（.grd 网格数据文件）
QString CFreqDomainDzToPoleDlg::askOpenFilePath(const QString& strTitle)
{
...
QString CFreqDomainDzToPoleDlg::askSaveFilePath(const QString& strTitle, const QString& strDefaultPath)
{
...
void CFreqDomainDzToPoleDlg::viewGridFile(const QString& strFilePath)
{
    if (strFilePath.isEmpty())
    {
        return;
...
// 功能：显示输入数据文件等值线
void CFreqDomainDzToPoleDlg::onOpenFileViewClicked()
{
    viewGridFile(mInputFilePath);
}

// 功能：显示处理结果文件等值线
void CFreqDomainDzToPoleDlg::onSaveFileViewClicked()
{
    viewGridFile(mOutputFilePath);
}

// 功能：扩边行数变化（同步成员变量）
void CFreqDomainDzToPoleDlg::onExRowsValueChanged(int nValue)
{
    mExRows = nValue;
}

// 功能：扩边列数变化（同步成员变量）
void CFreqDomainDzToPoleDlg::onExColsValueChanged(int nValue)
{
    mExCols = nValue;
}

// 功能：磁化偏角变化（同步成员变量）
void CFreqDomainDzToPoleDlg::onDtd0ValueChanged(double dValue)
{
    mDtd0 = dValue;
}

// 功能：磁化倾角变化（同步成员变量）
void CFreqDomainDzToPoleDlg::onDti0ValueChanged(double dValue)
{
    mDti0 = dValue;
}

// 功能：确定前校验输入项（对应原工程 OnOK 中的空值检查，并给出更明确的提示）
bool CFreqDomainDzToPoleDlg::validateInputs(QString& strError)
{
    // 输入数据文件检查
...
// 功能：组装参数并调用后端 processDzToPole（对应原工程 OnOK 主体逻辑）
void CFreqDomainDzToPoleDlg::runProcess()
{
    // 后端接口按需从全局服务获取（不缓存为成员指针）
    IRgisBackend* pBackend = CBackendService::rgisBackend();
    if (pBackend == NULL)
    ...
    bool bOk = pBackend->processDzToPole(params, error);
...
void CFreqDomainDzToPoleDlg::onOkClicked()
{
    QString strError;
...
void CFreqDomainDzToPoleDlg::onCancelClicked()
{
    reject();
}
