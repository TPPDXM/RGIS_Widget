// FreqDomainStructureDlg.h : 频率域构造（小子域滤波/线性构造增强）对话框（Qt 前端）
//
// 对应原 MFC 工程：CFreqDomainStructureDlg（IDD_FreqDomainStructure）
// 界面布局依据 RGISGravMagnDataProcessing.rc 中 IDD_FreqDomainStructure 绘制：
//   数据文件输入 / 网格数据信息 / 数据文件输出
//
// 前端职责（本对话框只做界面与交互，算法/数据读写在后端）：
//   1. 输入数据文件的选择、网格信息显示、默认输出文件名生成（_Str.grd）；
//   2. 无处理参数与扩边输入（与原工程一致，扩边处理在算法内部完成）；
//   3. “确定”时校验输入，并调用后端接口 processStructure 完成处理；
...
// 频率域构造（小子域滤波/线性构造增强）对话框
class CFreqDomainStructureDlg : public QDialog
{
    Q_OBJECT
...
public:
    ...
    explicit CFreqDomainStructureDlg(QWidget* pParent = nullptr);

    // 析构函数
    ~CFreqDomainStructureDlg() override;
...
private:
    ...
    void runProcess();                          // 功能：组装参数并调用后端 processStructure
    ...
    // ===== 界面控件（数据文件输出）=====
    QLineEdit*  mEditSaveFile;       // 处理结果数据文件名（默认 基准名 + "_Str.grd"）
